...
MAP_WAARNEMINGEN   <- here("data/input/Waarnemingen_Soorten/De_Maten")
...
# ------------------------------------------------------------------------------
# 1. DETECTEER EN KNIT DE UNIEKE DM-SCRIPTS (Runt ALLE unieke scripts)
# ------------------------------------------------------------------------------
alle_tv_scripts       <- list.files(path = MAP_SCRIPTS_DM, pattern = "\\.Rmd$", full.names = TRUE)
soorten_script_tv_pad <- file.path(MAP_SCRIPTS_DM, "DM_Leefgebieden_Simpel.Rmd")
unieke_tv_scripts     <- setdiff(alle_tv_scripts, soorten_script_tv_pad)

message("=> Gedetecteerde unieke DM-scripts om te knitten (Aantal: ", length(unieke_tv_scripts), "):")
...
# ------------------------------------------------------------------------------
# 3. RUN HET 'SIMPEL_DM' SCRIPT PER GEFILTERDE SOORT
# ------------------------------------------------------------------------------
