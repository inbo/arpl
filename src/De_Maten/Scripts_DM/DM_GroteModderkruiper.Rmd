...
if (!all(is.na(terra::values(cl_max, mat=FALSE)))) {
  ...
  colnames(zonal_zone_max) <- c("ID", "In_DM")
  ids_binnen_max <- zonal_zone_max$ID[!is.na(zonal_zone_max$In_DM) & zonal_zone_max$In_DM > 0]
  
  zonal_zone_opp <- terra::zonal(binnen_masker, cl_opp, fun = "max", na.rm = TRUE)
  colnames(zonal_zone_opp) <- c("ID", "In_DM")
  ids_binnen_opp <- zonal_zone_opp$ID[!is.na(zonal_zone_opp$In_DM) & zonal_zone_opp$In_DM > 0]
...
# ==============================================================================
# STAP 14: INTERACTIEVE KAART (GESYNCHRONISEERDE METRICA)
# ==============================================================================
if (!all(is.na(terra::values(cl_max, mat=FALSE)))) {
  
  # 1. Bereken Potentiële Oppervlakte (Spoor A) per uniek Cluster ID op cl_max
  # (alle_matches bevat de unieke 0.01 ha pixels binnen de clusters)
  stats_pot_corr <- terra::zonal(alle_matches * 0.01, cl_max, fun = "sum", na.rm = TRUE)
  colnames(stats_pot_corr) <- c("ID", "Pot_ha")

  # 2. Bereken Werkelijke Oppervlakte (Spoor B) per uniek Cluster ID op cl_max
  # (alle_oppervlaktes bevat de reële BWK/plas-fracties binnen de clusters)
  stats_werk_corr <- terra::zonal(alle_oppervlaktes * 0.01, cl_max, fun = "sum", na.rm = TRUE)
  colnames(stats_werk_corr) <- c("ID", "Werk_ha")

  # 3. Koppel de metrica op ID en voeg de zone/ARPL-status toe
  cluster_info <- stats_pot_corr %>%
    left_join(stats_werk_corr, by = "ID") %>%
    left_join(zonal_zone_max, by = "ID") %>%
    mutate(
      Pot_ha = round(Pot_ha, 2),
      Werk_ha = round(coalesce(Werk_ha, 0), 2),
      Zone = ifelse(!is.na(In_DM) & In_DM > 0, "Binnen DM", "Buiten DM (Buffer)"),
      ARPL = ID %in% ids_actueel_relevant,
      # Status bepalen: is het werkelijk habitat >= drempel?
      Type = ifelse(Werk_ha >= oppervlakte_ha, "Werkelijk Habitat", "Potentieel (Niet Werkelijk)"),
      Popup = paste0(
        ...
        "<strong>Potentieel Oppervlak:</strong> ", Pot_ha, " ha<br>",
        "<strong>Werkelijk Habitat:</strong> ", Werk_ha, " ha",
        "</div>"
      )
    )

  # 4. Vectoriseren naar Polygonen voor Leaflet
...
```{r export}
# ==============================================================================
# SCHONE BINAIR EXPORT ZONDER VALSE NULLEN (GROTE MODDERKRUIPER)
# ==============================================================================
message("-> Start geformatteerde binaire export voor grote modderkruiper...")
...
# 1. Maximale potentie
if (exists("grotemodderkruiper_leefgebied_max") && !all(is.na(terra::values(grotemodderkruiper_leefgebied_max, mat = FALSE)))) {
  potentie_export_rast <- terra::ifel(!is.na(grotemodderkruiper_leefgebied_max) & grotemodderkruiper_leefgebied_max > 0, 1, NA)
...
if (exists("grotemodderkruiper_leefgebied_opp") && !all(is.na(terra::values(grotemodderkruiper_leefgebied_opp, mat = FALSE)))) {
...
if (exists("grotemodderkruiper_verspreiding_bin") && !all(is.na(terra::values(grotemodderkruiper_verspreiding_bin, mat = FALSE)))) {
  verspreiding_export_rast <- terra::ifel(!is.na(grotemodderkruiper_verspreiding_bin) & grotemodderkruiper_verspreiding_bin > 0, 1, NA)
...
message("-> Alle GeoTIFF exports voor Grote modderkruiper zijn succesvol weggeschreven!")
```
