...
if (file.exists(watervlakken_path)) {
  ...
  if (!is.null(kleine_clusters_v) && nrow(kleine_clusters_v) > 0) {
    ...
    if (!is.null(kleine_watervlakken_finaal) && nrow(kleine_watervlakken_finaal) > 0) {
      r_rp_kleinwater <- terra::rasterize(kleine_watervlakken_finaal, template_DM, field = 1, background = NA)
...
if (!is.null(landbiotoop_bwk_max) && !all(is.na(suppressWarnings(terra::minmax(landbiotoop_bwk_max))))) {
  ...
  if (length(ids_land_ok_max) > 0) {
    ...
    rugstreeppad_land_gefilterd_max <- template_DM * NA
  }
} else {
  rugstreeppad_land_gefilterd_max <- template_DM * NA
}
...
if (!is.null(landbiotoop_bwk_opp) && !all(is.na(suppressWarnings(terra::minmax(landbiotoop_bwk_opp))))) {
  ...
  if (length(ids_land_ok_opp) > 0) {
    ...
    rugstreeppad_land_gefilterd_opp <- template_DM * NA
  }
} else {
  rugstreeppad_land_gefilterd_opp <- template_DM * NA
}
...
if (!is.null(rugstreeppad_land_gefilterd_max) && !all(is.na(suppressWarnings(terra::minmax(rugstreeppad_land_gefilterd_max)))) && 
  ...
  rugstreeppad_leefgebied1_max <- template_DM * NA
}
...
if (!is.null(rugstreeppad_land_gefilterd_opp) && !all(is.na(suppressWarnings(terra::minmax(rugstreeppad_land_gefilterd_opp)))) && 
  ...
  rugstreeppad_leefgebied1_opp <- template_DM * NA
}
...
if (!all(is.na(suppressWarnings(terra::minmax(cl_netwerk_max))))) {
  ...
  if (length(na.omit(valid_ids_max)) > 0) {
    ...
    rugstreeppad_leefgebied_max <- template_DM * NA
...
if (!all(is.na(suppressWarnings(terra::minmax(cl_netwerk_opp))))) {
  ...
  if (length(na.omit(valid_ids_opp)) > 0) {
    ...
    rugstreeppad_leefgebied_opp <- template_DM * NA
