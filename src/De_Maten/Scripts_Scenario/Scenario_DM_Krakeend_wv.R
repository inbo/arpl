...
# ==============================================================================
# STAP 2: WATERLOPEN INTEGREREN EN DIRECT LEEFGEBIED SAMENVOEGEN (GEEN DREMPELS)
# ==============================================================================
message("-> Huetzon-raster inlezen en alle geschikte waterlopen selecteren...")
r_huetzon_raw <- terra::rast(here("data/input/Raster_Vlaanderen/vlaanderen_huetzon_10m.tif"))
r_huetzon     <- terra::resample(r_huetzon_raw, id_raster_DM, method = "near") %>% terra::crop(id_raster_DM)
...
krakeend_leefgebied_max <- terra::mask(r_foer_max, r_watervogelgebied_max) %>% terra::crop(template_DM)
krakeend_leefgebied_opp <- terra::mask(r_foer_opp, r_watervogelgebied_max) %>% terra::crop(template_DM)

message("--- KRAKEEND MODEL SUCCESVOL AFGEROND ---")
cat("Totaal stabiel leefgebied Krakeend (ha):", round(calc_ha_exact(krakeend_leefgebied_opp), 2), "\n")
