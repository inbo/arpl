...
# Maak een masker op basis van template_DM waarbij alle geldige cellen de waarde 1 krijgen
r_watervogelgebied_max <- terra::ifel(!is.na(template_DM), 1, NA)

message("-> Waterlopen inlezen...")
r_waterlopen_raw <- rast(here("data/input/Raster_Vlaanderen/vlaanderen_huetzon_10m.tif"))
r_waterlopen     <- terra::resample(r_waterlopen_raw, id_raster_DM, method = "near") %>% terra::crop(id_raster_DM)
r_waterlopen_ok  <- terra::ifel(!is.na(r_waterlopen) & r_waterlopen > 0, 1, NA)

# 1. DIRECTE WATERHABITATS (BWK Water + Waterlopen = GEEN DREMPELS)
water_habitat_max <- terra::cover(water_bwk_max, r_waterlopen_ok)
water_habitat_opp <- terra::cover(water_bwk_opp, r_waterlopen_ok)

# 2. GRASLANDHABITATS (WEL 30 HA DREMPEL & 50M CLUSTERING)
drempel_m2 <- oppervlakte_ha * 10000

# Grasland Max Spoor
grasland_clusters_max <- cluster_filter_compleet(
  masker       = grasland_bwk_max,
  opp_laag     = grasland_bwk_opp,
  drempel_m2   = drempel_m2, 
  dist_m       = afstand_m, # 50 m
  werkelijk    = FALSE
)
r_grasland_30ha_max <- grasland_clusters_max$raster

# Grasland Opp Spoor
r_binair_grasland_opp <- terra::ifel(!is.na(grasland_bwk_opp) & grasland_bwk_opp > 0, 1, NA)

grasland_clusters_opp <- cluster_filter_compleet(
  masker       = r_binair_grasland_opp,
  opp_laag     = grasland_bwk_opp,
  drempel_m2   = drempel_m2, 
  dist_m       = afstand_m, # 50 m
  werkelijk    = TRUE
)
r_grasland_30ha_opp <- grasland_clusters_opp$raster

message("-> Water- en gefilterde graslandbiotopen samenvoegen tot leefgebied...")

# 3. COMBINEREN WATER + GRASLAND
finaal_reiger_max <- terra::cover(water_habitat_max, r_grasland_30ha_max)
finaal_reiger_opp <- terra::cover(water_habitat_opp, r_grasland_30ha_opp)

grotezilverreiger_leefgebied_max <- terra::mask(finaal_reiger_max, r_watervogelgebied_max) %>% terra::crop(template_DM)
grotezilverreiger_leefgebied_opp <- terra::mask(finaal_reiger_opp, r_watervogelgebied_max) %>% terra::crop(template_DM)

message("--- GROTE ZILVERREIGER MODEL SUCCESVOL AFGEROND ---")
cat("Totaal werkelijk habitat Grote Zilverreiger binnen regio (ha):", round(calc_ha_exact(grotezilverreiger_leefgebied_opp), 2), "\n")

rm(grasland_clusters_max, grasland_clusters_opp, r_grasland_30ha_max, r_grasland_30ha_opp, water_habitat_max, water_habitat_opp, r_watervogelgebied_max)
gc()
