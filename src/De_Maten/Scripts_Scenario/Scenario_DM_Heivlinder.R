...
groenkaart_10m_synchronized <- project(hooggroen_raw, id_raster_DM, method = "max")
...
if (!all(is.na(suppressWarnings(terra::minmax(voortplanting_base_max))))) {
  ...
  foerageer_finaal_max     <- template_DM * NA
  voortplanting_finaal_max <- template_DM * NA
}
...
if (!all(is.na(suppressWarnings(terra::minmax(voortplanting_base_opp))))) {
  ...
  foerageer_finaal_opp     <- template_DM * NA
  voortplanting_finaal_opp <- template_DM * NA
}
