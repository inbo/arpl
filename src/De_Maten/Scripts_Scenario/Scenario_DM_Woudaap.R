...
# --- SPOOR A: MAX ---
r_woudaap_voortplanting2_max <- id_raster_DM * NA
...
r_woudaap_voortplanting2_opp <- id_raster_DM * NA
...
# --- SPOOR A: MAX ---
r_woudaap_leefgebied1_max <- id_raster_DM * NA
...
# --- SPOOR B: OPP (Parallel & Autonoom) ---
r_woudaap_leefgebied1_opp <- id_raster_DM * NA
...
woudaap_finaal_max <- template_DM * NA
woudaap_finaal_opp <- template_DM * NA
...
if (!all(is.na(terra::values(r_woudaap_leefgebied1_max, mat=FALSE)))) {
  ...
  woudaap_finaal_max <- cl_woudaap_finaal_max$raster %>% terra::crop(template_DM)
}
...
if (!all(is.na(terra::values(r_woudaap_leefgebied1_opp, mat=FALSE)))) {
  ...
  woudaap_finaal_opp <- cl_woudaap_finaal_opp$raster %>% terra::crop(template_DM)
