...
if (!all(is.na(terra::values(r_voort_vlakbij_water_max, mat=FALSE))) && !all(is.na(terra::values(r_foer_200ha_max, mat=FALSE)))) {
  ...
  buffer_20km_voort_max <- terra::buffer(r_voort_vlakbij_water_max, width = 20000)
  
  r_voort_finaal_max <- terra::mask(r_voort_vlakbij_water_max, buffer_20km_foer_max)
  r_foer_finaal_max  <- terra::mask(r_foer_200ha_max, buffer_20km_voort_max)
  
  rm(buffer_20km_foer_max, buffer_20km_voort_max)
}
...
if (!all(is.na(terra::values(r_voort_vlakbij_water_opp, mat=FALSE))) && !all(is.na(terra::values(r_foer_200ha_opp, mat=FALSE)))) {
  r_binair_voort2_opp <- terra::ifel(!is.na(r_voort_vlakbij_water_opp) & r_voort_vlakbij_water_opp > 0, 1, NA)
  ...
  buffer_20km_voort_opp <- terra::buffer(r_binair_voort2_opp, width = 20000)
  
  r_voort_finaal_opp <- terra::mask(r_voort_vlakbij_water_opp, buffer_20km_foer_opp)
  r_foer_finaal_opp  <- terra::mask(r_foer_200ha_opp, buffer_20km_voort_opp)
  
  rm(r_binair_voort2_opp, r_binair_foer200_opp, buffer_20km_foer_opp, buffer_20km_voort_opp)
}

rm(foer_clusters_max, foer_clusters_opp, r_binair_foer1_opp, r_voort_vlakbij_water_max, r_voort_vlakbij_water_opp, 
   r_foer_200ha_max, r_foer_200ha_opp)
gc()
...
cat("Definitief leefgebied Zwartkopmeeuw MAX (ha):", round(calc_ha_exact(zwartkopmeeuw_leefgebied_max), 2), "\n")
cat("Definitief leefgebied Zwartkopmeeuw OPP (ha):", round(calc_ha_exact(zwartkopmeeuw_leefgebied_opp), 2), "\n")
