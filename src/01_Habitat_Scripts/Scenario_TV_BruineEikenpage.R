...
verwerk_habitat_met_bosrand <- function(r_habitat_max, r_habitat_opp, r_bos_max, r_bos_opp, dist_m = 20, drempel_m2, afstand_m = 0) {
  ...
  if (!terra::hasValues(r_habitat_max) || terra::global(r_habitat_max, "notNA")$notNA == 0) {
    return(list(max = template_TV * NA, opp = template_TV * NA, clusters_max = template_TV * NA, clusters_opp = template_TV * NA))
  }
...
if (terra::hasValues(ei_combo_max) && terra::hasValues(nectar_filter_max) &&
  ...
  bruineeikenpage_eiafzet_max <- template_TV * NA
  bruineeikenpage_nectar_max  <- template_TV * NA
  cl_eiafzet_max              <- template_TV * NA
  cl_nectar_max               <- template_TV * NA
}
...
if (terra::hasValues(ei_combo_opp) && terra::hasValues(nectar_filter_opp) &&
  ...
  cl_eiafzet_opp              <- template_TV * NA
  cl_nectar_opp               <- template_TV * NA
}
...
if (is.null(cl_max) || all(is.na(terra::values(cl_max, mat=FALSE)))) {
  if (!is.null(final_max) && !all(is.na(terra::values(final_max, mat=FALSE)))) {
