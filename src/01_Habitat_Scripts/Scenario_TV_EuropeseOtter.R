...
for(i in 1:nrow(resultaten_gegroepeerd)) {
  ...
  if(nrow(tabel_cel_som) > 0) {
    ...
    if(nrow(tabel_finaal_mapping) > 0) {
      r_opp_type   <- template_TV * NA
      r_match_type <- template_TV * NA
      
      vec_opp   <- rep(NA_real_, terra::ncell(id_raster_TV))
      vec_match <- rep(NA_real_, terra::ncell(id_raster_TV))
...
clean_max <- function(r) {
  if (is.null(r)) return(template_TV * NA)
  r[is.na(r) | r == 0] <- NA
  return(r)
}
...
if (!all(is.na(terra::values(otter_biotopen_max, mat = FALSE)))) {
  ...
  r_clusters_max <- terra::rasterize(poly_land_max, template_TV, field = "Cluster_ID", touches = FALSE)
  ...
  if (length(voldoet_ids_max) > 0) {
    ...
    otter_biotopen1000_max <- template_TV * NA
  }
...
if (!all(is.na(terra::values(r_binair_land_opp, mat = FALSE)))) {
  ...
  r_clusters_opp <- terra::rasterize(poly_land_opp, template_TV, field = "Cluster_ID", touches = FALSE)
  ...
  if (length(voldoet_ids_opp) > 0) {
    ...
    otter_biotopen1000_opp <- template_TV * NA
  }
...
if (exists("otter_water_max") && !all(is.na(terra::values(otter_water_max, mat=FALSE)))) {
  ...
  if (exists("otter_biotopen1000_max") && !all(is.na(terra::values(otter_biotopen1000_max, mat=FALSE)))) {
    ...
    final_max <- terra::ifel(otter_leefgebied_max == 1, 1, NA) %>% terra::crop(template_TV)
  ...
  if (!all(is.na(terra::values(r_binair_land1000_opp, mat=FALSE)))) {
    ...
    final_opp <- terra::crop(otter_leefgebied_opp, template_TV)
