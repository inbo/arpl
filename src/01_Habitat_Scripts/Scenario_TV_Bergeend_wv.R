...
foerageer_basis_opp <- terra::app(
  terra::sds(
    terra::ifel(is.na(foerageer_bwk_opp), 0, foerageer_bwk_opp),
    terra::ifel(is.na(r_waterlopen_bergeend), 0, terra::ifel(r_waterlopen_bergeend == 1, 1, 0))
  ),
  fun = function(x) pmin(sum(x, na.rm = TRUE), 1)
)
foerageer_basis_opp <- terra::ifel(!is.na(foerageer_basis_opp) & foerageer_basis_opp > 0, foerageer_basis_opp, NA)

rm(r_waterlopen_raw, r_waterlopen, wl_cats, ids_waterlopen_ok, r_waterlopen_bergeend, foerageer_bwk_max, foerageer_bwk_opp)

# ==============================================================================
# STAP 3: 50M CLUSTERING (5 HA) EN INTERSECTIE MET TELGEBIEDEN
# ==============================================================================
message("-> Landschapsclustering foerageerhabitat (50m fuzzy, drempel 5 ha)...")

drempel_m2 <- oppervlakte_ha * 10000

# --- SPOOR A: MAX ---
bergeend_clusters_max <- cluster_filter_compleet(
...
r_foer_5ha_max <- bergeend_clusters_max$raster

# --- SPOOR B: OPP ---
r_binair_foer_opp <- terra::ifel(!is.na(foerageer_basis_opp) & foerageer_basis_opp > 0, 1, NA)

bergeend_clusters_opp <- cluster_filter_compleet(
...
r_foer_5ha_opp <- bergeend_clusters_opp$raster

message("-> Intersectie: Alleen geschikt habitat BINNEN de vogelgebieden behouden...")
bergeend_leefgebied_max <- terra::mask(r_foer_5ha_max, r_watervogelgebied_max) %>% terra::crop(template_TV)
bergeend_leefgebied_opp <- terra::mask(r_foer_5ha_opp, r_watervogelgebied_max) %>% terra::crop(template_TV)

rm(bergeend_clusters_max, bergeend_clusters_opp, foerageer_basis_max, foerageer_basis_opp, r_foer_5ha_max, r_foer_5ha_opp, r_watervogelgebied_max, r_binair_foer_opp)
gc()

# ==============================================================================
# DEFINITIEVE TOEWIJSING AAN EXPORT VARIABELEN
# ==============================================================================
final_max <- bergeend_leefgebied_max
...
for(item in export_config) {
  suffix <- if ((exists("zoek_sleutel") && grepl("_wv$", zoek_sleutel)) || (exists("soort") && grepl("bergeend", soort, ignore.case = TRUE))) "_wv.tif" else ".tif"
