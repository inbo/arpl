...
# Expert-parameters Knoflookpad
opp_water_ha   <- 0.03   # MinOpp waterbiotoop (0,03 ha = 300 m2)
...
min_plassen    <- 4      # Minimaal aantal geschikte plassen in het netwerk
...
if (file.exists(watervlakken_path)) {
  ...
  if (!is.null(kleine_clusters_v) && nrow(kleine_clusters_v) > 0) {
    ...
    if (!is.null(kleine_watervlakken_finaal) && nrow(kleine_watervlakken_finaal) > 0) {
      r_kp_kleinwater <- terra::rasterize(kleine_watervlakken_finaal, template_TV, field = 1, background = NA)
...
suppressWarnings(
  rm(watervlakken_v, watervlakken_buffered, watervlakken_fuzzy_clusters, kleine_clusters_v, kleine_watervlakken_finaal, r_kp_kleinwater)
)
gc()
...
if (!is.null(landbiotoop_bwk_max) && !all(is.na(terra::values(landbiotoop_bwk_max, mat = FALSE)))) {
  ...
  if (length(ids_land_ok_max) > 0) {
    knoflookpad_land_gefilterd_max <- terra::mask(landbiotoop_bwk_max, land_patches_max %in% ids_land_ok_max)
  } else {
    knoflookpad_land_gefilterd_max <- template_TV * NA
  }
} else {
  knoflookpad_land_gefilterd_max <- template_TV * NA
}
...
if (!is.null(landbiotoop_bwk_opp) && !all(is.na(terra::values(landbiotoop_bwk_opp, mat = FALSE)))) {
  ...
  if (length(ids_land_ok_opp) > 0) {
    knoflookpad_land_gefilterd_opp <- terra::mask(landbiotoop_bwk_opp, land_patches_opp %in% ids_land_ok_opp)
  } else {
    knoflookpad_land_gefilterd_opp <- template_TV * NA
  }
} else {
  knoflookpad_land_gefilterd_opp <- template_TV * NA
}
...
# --- SPOOR MAX ---
if (!is.null(knoflookpad_land_gefilterd_max) && !all(is.na(terra::values(knoflookpad_land_gefilterd_max, mat = FALSE))) && 
    !is.null(r_knoflookpad_waterbiotoop1) && !all(is.na(terra::values(r_knoflookpad_waterbiotoop1, mat = FALSE)))) {
  
  r_land_mask_max  <- knoflookpad_land_gefilterd_max
  r_water_mask_max <- r_knoflookpad_waterbiotoop1
  
  kp_land_gekoppeld_max  <- terra::mask(knoflookpad_land_gefilterd_max, r_water_mask_max)
  kp_water_gekoppeld_max <- terra::mask(r_knoflookpad_waterbiotoop1, r_land_mask_max)
  
  knoflookpad_leefgebied1_max <- kp_land_gekoppeld_max | kp_water_gekoppeld_max
  knoflookpad_leefgebied1_max <- terra::ifel(knoflookpad_leefgebied1_max == 1, 1, NA)
} else {
  knoflookpad_leefgebied1_max <- template_TV * NA
}

# --- SPOOR OPP ---
if (!is.null(knoflookpad_land_gefilterd_opp) && !all(is.na(terra::values(knoflookpad_land_gefilterd_opp, mat = FALSE))) && 
    !is.null(r_knoflookpad_waterbiotoop1_opp) && !all(is.na(terra::values(r_knoflookpad_waterbiotoop1_opp, mat = FALSE)))) {
  ...
  kp_land_gekoppeld_opp  <- terra::mask(knoflookpad_land_gefilterd_opp, r_water_mask_opp)
  kp_water_gekoppeld_opp <- terra::mask(r_knoflookpad_waterbiotoop1_opp, r_land_mask_opp)
  ...
  knoflookpad_leefgebied1_opp <- template_TV * NA
}

message("-> Netwerkvorming binnen 1000m en controle op >= 160 ha EN >= 4 plassen voor MAX en OPP parallel...")
...
if (!all(is.na(terra::values(cl_netwerk_max, mat = FALSE)))) {
  ...
  if (length(na.omit(valid_ids_max)) > 0) {
    knoflookpad_leefgebied_max <- terra::mask(r_netwerk_max, cl_netwerk_max %in% valid_ids_max)
    cl_max                     <- terra::mask(cl_netwerk_max, cl_netwerk_max %in% valid_ids_max)
  } else {
    knoflookpad_leefgebied_max <- template_TV * NA
...
res_netwerk_opp <- cluster_filter_compleet(
  ...
  opp_laag   = knoflookpad_leefgebied1_opp,
  drempel_m2 = drempel_netwerk_m2,
  dist_m     = buffer_m,
  werkelijk  = TRUE
)
...
if (!all(is.na(terra::values(cl_netwerk_opp, mat = FALSE)))) {
  ...
  if (length(na.omit(valid_ids_opp)) > 0) {
    knoflookpad_leefgebied_opp <- terra::mask(knoflookpad_leefgebied1_opp, cl_netwerk_opp %in% valid_ids_opp)
    cl_opp                     <- terra::mask(cl_netwerk_opp, cl_netwerk_opp %in% valid_ids_opp)
  } else {
    knoflookpad_leefgebied_opp <- template_TV * NA
