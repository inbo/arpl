...
afstand_biotoop_m <- resultaat$AfstandBiotopen_m[1]
min_opp_m2        <- resultaat$MinOpp_ha[1] * 10000
...
resultaten_gegroepeerd <- df_nieuw %>%
  ...
  filter(Soort_clean == soort, Type == "voortplanting") %>%
  group_by(Type) %>%
...
voortplanting_max <- id_raster_TV * NA
voortplanting_opp <- id_raster_TV * NA

if(nrow(resultaten_gegroepeerd) > 0) {
  h_data <- resultaten_gegroepeerd$Data[[1]]
  ...
  if(nrow(tabel_cel_som) > 0) {
    ...
    if(nrow(tabel_finaal_mapping) > 0) {
      ...
      if(nrow(tabel_matches_clean) > 0) {
        voortplanting_max[tabel_matches_clean$lokale_id] <- tabel_matches_clean$Match
      }
      voortplanting_opp[tabel_finaal_mapping$lokale_id] <- tabel_finaal_mapping$Oppervlakte
    }
  }
}

rm(resultaten_gegroepeerd, df_nieuw, tabel_vlaanderen, vertaal_df)
gc()

# ==============================================================================
# DIRECTE CLUSTERING VAN VOORTPLANTINGSHABITAT
# ==============================================================================
message("-> Berekenen leefgebied hazelmuis rechtstreeks uit voortplantingshabitat...")

# Spoor A: MAX
leefgebied_finaal_geclusterd_max <- cluster_filter_compleet(
  masker = voortplanting_max, 
  opp_laag = voortplanting_opp, 
  drempel_m2 = min_opp_m2, 
  dist_m = afstand_biotoop_m, 
  werkelijk = FALSE
)
...
if (!all(is.na(terra::values(final_opp, mat=FALSE)))) {
  cl_opp <- leefgebied_finaal_geclusterd_opp$clusters
} else {
  cl_opp <- template_TV * NA
}

if (!all(is.na(terra::values(final_max, mat=FALSE)))) {
  cl_max <- leefgebied_finaal_geclusterd_max$clusters
} else {
  cl_max <- template_TV * NA
}

rm(r_binair_vp_opp, leefgebied_finaal_geclusterd_max, leefgebied_finaal_geclusterd_opp)
gc()
