...
if (is_hts_exclusief) {
  ...
  r_scen_raw <- werkelijk_export_rast
...
# HARD RESAMPLE & CROP VOOR REF EN SCEN OM EXTENT MISMATCHES VOLLEDIG TE VERMIJDEN
if (!is.null(referentie_file) && file.exists(referentie_file)) {
  r_ref_raw <- rast(referentie_file) %>% 
    terra::crop(template_TV, snap = "near") %>% 
    terra::resample(template_TV, method = "near")
  r_ref <- terra::ifel(!is.na(r_ref_raw) & r_ref_raw == 1, 1, NA)
} else {
...
r_scen <- r_scen_raw %>% 
  terra::crop(template_TV, snap = "near") %>% 
  terra::resample(template_TV, method = "near")
r_scen <- terra::ifel(!is.na(r_scen) & r_scen > 0, 1, NA)

# OPMERKING: Beide rasters hebben nu gegarandeerd 100% exact dezelfde raster-extent & dimensies
...
```{r export}
# ==============================================================================
# 2. SCHONE BINAIR EXPORT ZONDER VALSE NULLEN (SCENARIO HOOGVEENGLANSLIBEL)
# ==============================================================================
...
message("🏁 SCENARIO EXPORT VOLLEDIG AFGEROND VOOR HOOGVEENGLANSLIBEL!")
```
