...
# Expliciete declaratie van de 4 rasters (Totaal leefgebied: Kern + Bouwsteen)
potentie_export_rast     <- if(exists("grutto_kaart_A") && !is.null(grutto_kaart_A) && !all(is.na(terra::values(grutto_kaart_A, mat=FALSE)))) terra::ifel(grutto_kaart_A > 0, 1, NA) else terra::rast(template_TV, vals = NA)
werkelijk_export_rast    <- if(!is.null(resB_strikt) && (!all(is.na(terra::values(resB_strikt$kern, mat=FALSE))) \vert{}\vert{} !all(is.na(terra::values(resB_strikt$bouw, mat=FALSE))))) terra::ifel(!is.na(terra::cover(resB_strikt$kern, resB_strikt$bouw)) & terra::cover(resB_strikt$kern, resB_strikt$bouw) > 0, 1, NA) else terra::rast(template_TV, vals = NA)
arpl_export_rast         <- if(exists("grutto_arpl_opp") && !is.null(grutto_arpl_opp) && !all(is.na(terra::values(grutto_arpl_opp, mat=FALSE)))) terra::ifel(grutto_arpl_opp > 0, 1, NA) else terra::rast(template_TV, vals = NA)
verspreiding_export_rast <- if(exists("grutto_verspreiding_bin") && !is.null(grutto_verspreiding_bin) && !all(is.na(terra::values(grutto_verspreiding_bin, mat=FALSE)))) terra::ifel(grutto_verspreiding_bin > 0, 1, NA) else terra::rast(template_TV, vals = NA)
