---
title: "Scenario Leefgebiedenscript: Vliegend hert"
...
potentie_export_rast     <- if(exists("final_max") && !all(is.na(terra::values(final_max, mat=FALSE)))) terra::ifel(!is.na(final_max) & final_max > 0, 1, NA) else terra::rast(template_VS, vals = NA)
werkelijk_export_rast    <- if(exists("final_opp") && !all(is.na(terra::values(final_opp, mat=FALSE)))) terra::ifel(!is.na(final_opp) & final_opp > 0, 1, NA) else terra::rast(template_VS, vals = NA)
arpl_export_rast         <- if(exists("arpl_raster") && !all(is.na(terra::values(arpl_raster, mat=FALSE)))) terra::ifel(arpl_raster > 0, 1, NA) else terra::rast(template_VS, vals = NA)
verspreiding_export_rast <- if(exists("verspreiding_raster") && !all(is.na(terra::values(verspreiding_raster, mat=FALSE)))) terra::ifel(verspreiding_raster > 0, 1, NA) else terra::rast(template_VS, vals = NA)
...
if (!is.null(referentie_file) && file.exists(referentie_file)) {
  r_ref_raw <- rast(referentie_file) %>% terra::resample(template_VS, method = "near")
  r_ref <- terra::ifel(r_ref_raw == 1, 1, NA)
} else {
  message("⚠️ Waarschuwing: Geen referentiebestand gevonden op schijf.")
  r_ref <- terra::rast(template_VS, vals = NA)
}
...
```{r export}
# ==============================================================================
# 2. SCHONE BINAIR EXPORT ZONDER VALSE NULLEN (SCENARIO VLIEGEND HERT)
# ==============================================================================
...
message("🏁 SCENARIO EXPORT VOLLEDIG AFGEROND VOOR VLIEGEND HERT!")
```
