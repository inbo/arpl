...
# BEHOUD vertaal_df voor de terrils in de volgende chunk
...
# FAILSAFE: Heropbouw van vertaal_df als deze door een eerdere stap zou ontbreken
if (!exists("vertaal_df")) {
  id_raster_TV_masked <- mask(crop(master_grid, area_buffer_fix, snap = "near"), area_buffer_fix)
  df_extractie <- as.data.frame(id_raster_TV_masked, cells = TRUE)
  vertaal_df <- as.data.table(df_extractie)
  setnames(vertaal_df, c(1, 2), c("lokale_id", "globale_id"))
  vertaal_df <- vertaal_df[!is.na(globale_id)]
  rm(df_extractie, id_raster_TV_masked)
}

# 1. HOOGGROEN SYNCHRONISEREN EN HARD WEGSCHRIJVEN NAAR SCHIJF
...
# Nu vertaal_df veilig opruimen nadat terrils zijn verwerkt
...
potentie_export_rast     <- if(exists("biotoop_fase7_bin") && !all(is.na(terra::values(biotoop_fase7_bin, mat=FALSE)))) terra::ifel(!is.na(biotoop_fase7_bin) & biotoop_fase7_bin > 0, 1, NA) else terra::rast(template_TV, vals = NA)
werkelijk_export_rast    <- if(exists("biotoop_fase7_frac") && !all(is.na(terra::values(biotoop_fase7_frac, mat=FALSE)))) terra::ifel(!is.na(biotoop_fase7_frac) & biotoop_fase7_frac > 0, 1, NA) else terra::rast(template_TV, vals = NA)
arpl_export_rast         <- if(exists("kwartelkoning_arpl_opp") && !all(is.na(terra::values(kwartelkoning_arpl_opp, mat=FALSE)))) terra::ifel(kwartelkoning_arpl_opp > 0, 1, NA) else terra::rast(template_TV, vals = NA)
verspreiding_export_rast <- if(exists("kwartelkoning_verspreiding_bin") && !all(is.na(terra::values(kwartelkoning_verspreiding_bin, mat=FALSE)))) terra::ifel(kwartelkoning_verspreiding_bin > 0, 1, NA) else terra::rast(template_TV, vals = NA)
...
if (!is.null(referentie_file) && file.exists(referentie_file)) {
  ...
  r_ref <- terra::ifel(r_ref_raw > 0, 1, NA)
} else {
  message("⚠️ Waarschuwing: Geen referentiebestand gevonden op schijf.")
  r_ref <- terra::rast(template_TV, vals = NA)
}

r_scen_bin <- terra::ifel(!is.na(r_scen) & r_scen > 0, 1, NA)

r_rgb <- terra::ifel(!is.na(r_ref) & is.na(r_scen_bin), 1, NA)
r_rgb <- terra::cover(r_rgb, terra::ifel(is.na(r_ref) & !is.na(r_scen_bin), 2, NA))
r_rgb <- terra::cover(r_rgb, terra::ifel(!is.na(r_ref) & !is.na(r_scen_bin), 3, NA))
...
```{r export}
# ==============================================================================
# 2. SCHONE BINAIR EXPORT ZONDER VALSE NULLEN (SCENARIO KWARTELKONING)
# ==============================================================================
...
message("🏁 SCENARIO EXPORT VOLLEDIG AFGEROND VOOR KWARTELKONING!")
```
