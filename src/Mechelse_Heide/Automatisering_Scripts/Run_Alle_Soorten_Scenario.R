...
# ------------------------------------------------------------------------------
GEBIED_NAAM      <- "Mechelse_Heide"
GEBIED_CODE      <- "MH"

SCENARIO_RDS_PAD <- here("data/input/Scenario_rds/MH_Scenario_BWK_2025.rds")
HUIDIG_SCENARIO  <- gsub("^MH_Scenario_|.rds$", "", basename(SCENARIO_RDS_PAD), ignore.case = TRUE)
...
soorten_script_pad <- alle_scripts[grep("Scenario_MH_Leefgebieden_Simpel\\.r$", alle_scripts, ignore.case = TRUE)]

if (length(soorten_script_pad) == 0) {
  soorten_script_pad <- file.path(MAP_SCRIPTS, "Scenario_MH_Leefgebieden_Simpel.R")
} else {
