# ==============================================================================
# MASTER AUTOMATISERINGSSCRIPT VOOR REGULIERE MH-SCRIPTS (KNITTEN)
...
# ------------------------------------------------------------------------------
# 1. DETECTEER EN KNIT DE UNIEKE MH-SCRIPTS (Runt ALLE unieke scripts)
# ------------------------------------------------------------------------------
...
for(script in unieke_tv_scripts) {
  ...
  if (!moet_knitten(output_html, OUTPUT_DIR)) {
    ...
    algemeen_logboek[[bestandsnaam]] <- data.frame(
      Item = bestandsnaam, Type = "Uniek MH Script", Status = "GEKOZEN_OVERSLAAN", Fout = "Reeds vandaag geknit", stringsAsFactors = FALSE
    )
  ...
  tryCatch({
    ...
    algemeen_logboek[[bestandsnaam]] <- data.frame(
      Item = bestandsnaam, Type = "Uniek MH Script", Status = "SUCCES", Fout = "Geen", stringsAsFactors = FALSE
    )
  }, error = function(e) {
    message("❌ FOUT bij: ", bestandsnaam)
    algemeen_logboek[[bestandsnaam]] <- data.frame(
      Item = bestandsnaam, Type = "Uniek MH Script", Status = "CRASH", Fout = e$message, stringsAsFactors = FALSE
    )
  })
}
...
# ------------------------------------------------------------------------------
# 3. RUN HET 'SIMPEL_MH' SCRIPT PER GEFILTERDE SOORT
# ------------------------------------------------------------------------------
draai_leefgebied_model <- function(huidige_soort, script_pad) {
  output_file_name <- paste0("MH_", huidige_soort, ".html")
...
# ------------------------------------------------------------------------------
log_file_path <- file.path(OUTPUT_DIR, paste0("Logboek_MH_NormaalRun_", format(Sys.time(), "%Y%m%d_%H%M"), ".csv"))
...
message("\n==================================================")
message(" MASTER RUN MH NORMAAL COMPLEET!                  ")
