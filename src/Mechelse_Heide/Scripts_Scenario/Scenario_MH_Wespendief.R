...
if (!all(is.na(terra::values(r_foerageer2_max, mat=FALSE)))) {
  ...
  wespendief_leefgebied_max <- terra::mask(r_wespendief_bos_max, buffer_5000m_foer_max) %>% terra::crop(template_MH)
  suppressWarnings(rm(buffer_5000m_foer_max))
} else {
  wespendief_leefgebied_max <- template_MH * NA
}
...
if (!all(is.na(terra::values(r_foerageer2_opp, mat=FALSE)))) {
  ...
  wespendief_leefgebied_opp <- terra::mask(r_wespendief_bos_opp, buffer_5000m_foer_opp) %>% terra::crop(template_MH)
  suppressWarnings(rm(r_binair_foer2_opp, buffer_5000m_foer_opp))
} else {
  wespendief_leefgebied_opp <- template_MH * NA
}
