...
waterbiotoop_bwk_max <- if (!is.na(w_key)) lijst_matches[[w_key]] else template_MH * NA
waterbiotoop_bwk_opp <- if (!is.na(w_key)) lijst_oppervlaktes[[w_key]] else template_MH * NA
landbiotoop_bwk_max  <- if (!is.na(l_key)) lijst_matches[[l_key]] else template_MH * NA
landbiotoop_bwk_opp  <- if (!is.na(l_key)) lijst_oppervlaktes[[l_key]] else template_MH * NA
...
if (file.exists(watervlakken_path)) {
  ...
  if (!is.null(kleine_clusters_v) && nrow(kleine_clusters_v) > 0) {
    ...
    if (!is.null(kleine_watervlakken_finaal) && nrow(kleine_watervlakken_finaal) > 0) {
      r_hk_kleinwater <- terra::rasterize(kleine_watervlakken_finaal, template_MH, field = 1, background = NA)
