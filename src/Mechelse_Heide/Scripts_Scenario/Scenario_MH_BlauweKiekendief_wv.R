...
cluster_filter_compleet <- function(masker, opp_laag, drempel_m2, dist_m, werkelijk = FALSE, straal_m = 0) {
  ...
  if (exists("id_raster_MH")) {
    masker   <- terra::crop(masker, id_raster_MH, snap = "out")
    opp_laag <- terra::crop(opp_laag, id_raster_MH, snap = "out")
  }
...
id_raster_MH <- crop(master_grid, area_buffer_fix, snap = "out")
values(id_raster_MH) <- 1:terra::ncell(id_raster_MH)

globale_ids_vector <- terra::cells(master_grid, terra::ext(id_raster_MH))

vertaal_df <- data.table(
  lokale_id  = 1:terra::ncell(id_raster_MH),
...
template_MH <- terra::rasterize(area_buffer_fix, id_raster_MH, field = 1, background = NA)
...
tabel_bebouw_MH  <- tabel_vlaanderen[CODE %in% bebouwings_lagen & cel_id %in% studiegebied_globale_ids]
tabel_bebouw_som <- tabel_bebouw_MH[, .(Present = ifelse(sum(BWK_FRAC, na.rm = TRUE) >= 0.3, 1, 0)), by = .(cel_id)]
...
if (!is.null(r_bebouw_groot) && !terra::global(is.na(r_bebouw_groot), "sum")[[1]] == terra::ncell(r_bebouw_groot)) {
  ...
  masker_bebouw_100m <- id_raster_MH * NA
}
...
# 1. Lees de hooggroen-laag (Band 1) in en snijd direct af op id_raster_MH
r_hooggroen_frac <- rast(here("data/input/Raster_Vlaanderen/Groenkaart_2021_Fracties_5Bands_10m.tif"))[[1]] %>% 
  terra::crop(id_raster_MH, snap = "out")
...
r_bos_10m_back <- terra::disagg(verbossing_clusters_40m$raster, fact = 4) %>% 
  terra::crop(id_raster_MH, snap = "near")
