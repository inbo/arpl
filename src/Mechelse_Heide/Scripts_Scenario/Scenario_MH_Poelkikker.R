...
if (!is.null(kleine_clusters_v) && nrow(kleine_clusters_v) > 0) {
  ...
  if (!is.null(kleine_watervlakken_finaal) && nrow(kleine_watervlakken_finaal) > 0) {
    r_pk_kleinwater <- terra::rasterize(kleine_watervlakken_finaal, template_MH, field = 1, background = NA)
...
if (!all(is.na(terra::values(landbiotoop_bwk_max, mat = FALSE)))) {
  ...
  if (length(ids_land_ok_max) > 0) {
    ...
    poelkikker_land_gefilterd_max <- template_MH * NA
  }
} else {
  poelkikker_land_gefilterd_max <- template_MH * NA
}
...
if (!all(is.na(terra::values(landbiotoop_bwk_opp, mat = FALSE)))) {
  ...
  if (length(ids_land_ok_opp) > 0) {
    ...
    poelkikker_land_gefilterd_opp <- template_MH * NA
  }
} else {
  poelkikker_land_gefilterd_opp <- template_MH * NA
}
...
if (!all(is.na(terra::values(poelkikker_land_gefilterd_max, mat = FALSE))) && 
  ...
  poelkikker_leefgebied1_max <- template_MH * NA
}
...
if (!all(is.na(terra::values(poelkikker_land_gefilterd_opp, mat = FALSE))) && 
  ...
  poelkikker_leefgebied1_opp <- template_MH * NA
}
...
if (!all(is.na(terra::values(cl_netwerk_max, mat = FALSE)))) {
  ...
  if (length(na.omit(valid_ids_max)) > 0) {
    ...
    poelkikker_leefgebied_max <- template_MH * NA
...
if (!all(is.na(terra::values(cl_netwerk_opp, mat = FALSE)))) {
  ...
  if (length(na.omit(valid_ids_opp)) > 0) {
    ...
    poelkikker_leefgebied_opp <- template_MH * NA
