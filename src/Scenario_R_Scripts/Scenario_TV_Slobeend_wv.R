...
# ==============================================================================
# STAP 2: SLOBEEND - LANDSCHAPSCLUSTERING EN KRUISING (5 HA - PARALLEL)
# ==============================================================================
message("-> Landschapsclustering op foerageerhabitat (50m fuzzy, drempel 5 ha)...")
drempel_m2 <- oppervlakte_ha * 10000

# --- SPOOR A: MAXIMALE POTENTIE (Binair) ---
slobeend_clusters_max <- cluster_filter_compleet(
...
r_foer_5ha_max <- slobeend_clusters_max$raster
...
slobeend_clusters_opp <- cluster_filter_compleet(
...
r_foer_5ha_opp <- slobeend_clusters_opp$raster

slobeend_leefgebied_max <- terra::mask(r_foer_5ha_max, r_watervogelgebied_max) %>% terra::crop(template_TV)
slobeend_leefgebied_opp <- terra::mask(r_foer_5ha_opp, r_watervogelgebied_max) %>% terra::crop(template_TV)

# DEFINITIEVE MODELUITGANGEN
final_max <- slobeend_leefgebied_max
final_opp <- slobeend_leefgebied_opp

if (!all(is.na(suppressWarnings(terra::minmax(final_opp))))) {
  cl_opp <- slobeend_clusters_opp$clusters
} else {
  cl_opp <- template_TV * NA
}

if (!all(is.na(suppressWarnings(terra::minmax(final_max))))) {
  cl_max <- slobeend_clusters_max$clusters
} else {
  cl_max <- template_TV * NA
}

rm(r_binair_foer_opp, slobeend_clusters_max, slobeend_clusters_opp, r_foer_5ha_max, r_foer_5ha_opp, r_watervogelgebied_max)
gc()
...
for(item in export_config) {
  suffix <- if ((exists("zoek_sleutel") && grepl("_wv$", zoek_sleutel)) || (exists("soort") && grepl("slobeend", soort, ignore.case = TRUE))) "_wv.tif" else ".tif"
