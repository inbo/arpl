...
# ==============================================================================
# STAP 2: WATERLOPEN INTEGREREN EN CLUSTERING (5 HA)
# ==============================================================================
...
final_max <- tafeleend_leefgebied_max
final_opp <- tafeleend_leefgebied_opp

if (!all(is.na(suppressWarnings(terra::minmax(final_opp))))) {
  cl_opp <- tafeleend_clusters_opp$clusters
} else {
  cl_opp <- template_TV * NA
}

if (!all(is.na(suppressWarnings(terra::minmax(final_max))))) {
  cl_max <- tafeleend_clusters_max$clusters
} else {
  cl_max <- template_TV * NA
}

rm(r_huetzon_raw, r_huetzon, huetzon_cats, ids_waterlopen_ok, r_waterlopen, r_waterlopen_opp, 
   foerageer_compleet_max, foerageer_compleet_opp, r_binair_foer_opp, tafeleend_clusters_max, tafeleend_clusters_opp, r_foer_5ha_max, r_foer_5ha_opp, r_watervogelgebied_max)
gc()
...
for(item in export_config) {
  suffix <- if ((exists("zoek_sleutel") && grepl("_wv$", zoek_sleutel)) || (exists("soort") && grepl("tafeleend", soort, ignore.case = TRUE))) "_wv.tif" else ".tif"
