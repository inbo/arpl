...
# ==============================================================================
# STAP 2: WATERLOPEN INTEGREREN, CLUSTERING EN GEBIEDSKRUISING
# ==============================================================================
...
if (!all(is.na(suppressWarnings(terra::minmax(final_opp))))) {
  cl_opp <- pijlstaart_clusters_opp$clusters
} else {
  cl_opp <- template_TV * NA
}

if (!all(is.na(suppressWarnings(terra::minmax(final_max))))) {
  cl_max <- pijlstaart_clusters_max$clusters
} else {
...
for(item in export_config) {
  suffix <- if ((exists("zoek_sleutel") && grepl("_wv$", zoek_sleutel)) || (exists("soort") && grepl("pijlstaart", soort, ignore.case = TRUE))) "_wv.tif" else ".tif"
