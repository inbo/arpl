...
if (exists("heide_max") && !all(is.na(suppressWarnings(terra::minmax(heide_max))))) {
...
if (!all(is.na(suppressWarnings(terra::minmax(final_opp))))) {
  cl_opp <- cl_open_land_opp$clusters
} else {
  cl_opp <- template_TV * NA
}

if (!all(is.na(suppressWarnings(terra::minmax(final_max))))) {
  cl_max <- cl_open_land_max$clusters
} else {
  cl_max <- template_TV * NA
}

rm(cl_open_land_max, cl_open_land_opp, r_open_land_finaal_max, r_open_land_finaal_opp, open_land_max, open_land_opp, r_binair_open_opp)
gc()
...
for(item in export_config) {
  suffix <- if ((exists("zoek_sleutel") && grepl("_wv$", zoek_sleutel)) || (exists("soort") && grepl("regenwulp", soort, ignore.case = TRUE))) "_wv.tif" else ".tif"
