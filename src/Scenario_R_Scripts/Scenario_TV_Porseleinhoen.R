...
rm(biotoop_max, biotoop_opp_clean_gras, biotoop_opp_clean_moer, biotoop_opp_som, biotoop_opp_raw, biotoop_opp, 
   ...
   vertaal_df, lijst_matches, lijst_oppervlaktes, tabel_vlaanderen, resultaten_gegroepeerd, df_nieuw)
gc()

message("=== STAP 2: Beperken tot natte gronden via Drainagekaart (PARALLEL) ===")

r_drain_raw   <- terra::rast(here("data/input/Raster_Vlaanderen/vlaanderen_drainage_n2khab_10m.tif"))
...
rm(r_drain_raw, r_drain_local, masker_drainage, mal_drainage, drain_cats, porseleinhoen_geschikt2_max, porseleinhoen_geschikt2_opp)
gc()
...
rm(cluster_3ha_max, cluster_3ha_opp, r_binair_geschikt3_opp, porseleinhoen_geschikt3_max, porseleinhoen_geschikt3_opp)
gc()
...
rm(r_groen_bin, bos_polys, groenkaart, groenkaart_TV, hooggroen_aligned, porseleinhoen_geschikt4_max, porseleinhoen_geschikt4_opp)
...
if (!all(is.na(suppressWarnings(terra::minmax(final_opp))))) {
  cl_opp <- finaal_10ha_opp$clusters
} else {
  cl_opp <- template_TV * NA
}

if (!all(is.na(suppressWarnings(terra::minmax(final_max))))) {
  cl_max <- finaal_10ha_max$clusters
} else {
  cl_max <- template_TV * NA
}

rm(finaal_10ha_max, finaal_10ha_opp, r_binair_geschikt5_opp, porseleinhoen_geschikt5_max, porseleinhoen_geschikt5_opp)
gc()
