...
# 2. GRASLAND-KOPPELING
if (!all(is.na(suppressWarnings(terra::minmax(water_totaal_max)))) && !is.null(grasland_bwk_max)) {
...
if (!all(is.na(suppressWarnings(terra::minmax(grasland_gekoppeld_max))))) {
...
drempel_water_m2 <- oppervlakte_ha * 10000
...
final_max <- smient_leefgebied_max
final_opp <- smient_leefgebied_opp
...
for(item in export_config) {
  suffix <- if ((exists("zoek_sleutel") && grepl("_wv$", zoek_sleutel)) || (exists("soort") && grepl("smient", soort, ignore.case = TRUE))) "_wv.tif" else ".tif"
