...
# --- STAP 13: INTERACTIEVE KAART (CORRECTIE ARPL_STATUS CHECK) ---
...
if(!is.null(resB_strikt) && !all(is.na(terra::values(resB_strikt$kern, mat=FALSE)))) {
  ...
  # Koppel de Netwerk_ID's ruimtelijk terug aan de kaartpolygonen
  ...
  # 3. Transformeer de waarnemingszone
  smear_web <- sf::st_transform(waarneming_smear_sf, 4326)
  
  # VEILIGE CHECK: Controleer of smear_web niet leeg is voor intersectie
  has_smear_data <- nrow(smear_web) > 0 && !sf::st_is_empty(smear_web)
  
  if (has_smear_data) {
    arpl_vector <- sf::st_intersects(poly_finaal$geometry, smear_web, sparse = FALSE)[, 1]
  } else {
    arpl_vector <- rep(FALSE, nrow(poly_finaal))
  }
  
  # 4. Bouw popups en attributen veilig op
  poly_finaal <- poly_finaal %>% mutate(
    Row_ID = 1:n(),
    ARPL_Status = arpl_vector,
    ...
    Popup = paste0(
      ...
      ifelse(ARPL_Status, 
             "<span style='color:green; font-weight:bold;'>JA (Raakt waarnemingszone)</span>", 
...
```{r export}
# ==============================================================================
# SCHONE BINAIR EXPORT ZONDER VALSE NULLEN (GRUTTO - TURNHOUTS VENNEGEBIED)
# ==============================================================================
...
if (exists("grutto_arpl_opp") && !is.null(grutto_arpl_opp) && !all(is.na(terra::values(grutto_arpl_opp, mat = FALSE)))) {
  arpl_export_rast <- terra::ifel(!is.na(grutto_arpl_opp) & grutto_arpl_opp > 0, 1, NA)
} else {
...
if (exists("grutto_verspreiding_bin") && !is.null(grutto_verspreiding_bin) && !all(is.na(terra::values(grutto_verspreiding_bin, mat = FALSE)))) {
  verspreiding_export_rast <- terra::ifel(!is.na(grutto_verspreiding_bin) & grutto_verspreiding_bin > 0, 1, NA)
} else {
...
export_config <- list(
  ...
  list(rast = werkelijk_export_rast_clean, naam = "Werkelijke_Oppervlaktes", folder = folders$werkelijk), 
...
message("✓ Grutto-kaarten (Totaal Leefgebied) succesvol geëxporteerd met transparante NA-achtergrond!")
```
