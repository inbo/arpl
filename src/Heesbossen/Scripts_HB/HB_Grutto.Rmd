...
# 1. Gefilterde territoria-CSV inlezen (Python export op broedcode >= 4)
waarnemingen_path <- here("data/input/Territoria_Soorten/HB_Territoria_Grutto.csv")
...
# Failsafe opvangsysteem bij afwezigheid van territoria
if (!has_observations) {
...
if (has_observations && !is.null(resB_strikt) && (!all(is.na(terra::values(resB_strikt$kern, mat=FALSE))) || !all(is.na(terra::values(resB_strikt$bouw, mat=FALSE))))) {
  ...
  # Het TOTALE LEEFGEBIED (Kern + Bijkomend Habitat/Bouwsteen gecombineerd op het Wereldfractie/Opp-spoor)
...
get_classified_netwerk_stats <- function(res_strikt) {
  ...
  # 3. Maak een binaire raster-masker van de Heesbossen grens (1 = Binnen HB, NA = Buiten HB)
  ...
  # 4. Bepaal welke unieke Netwerk-IDs minstens 1 pixel BINNEN het HB-gebied hebben liggen
  ...
  # Binnen HB: Al het habitat van netwerken die (deels) binnen HB liggen
  ...
  # Buiten HB: Al het habitat van netwerken die volledig buiten HB liggen
...
grutto_audit_tabel %>% 
  knitr::kable(caption = "Gevalideerde verdeling van Grutto-netwerken (Berekend op pixel-overlap met HB-grens)") %>% 
