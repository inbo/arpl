...
final_max_layer <- if(exists("leefgebied_max")) leefgebied_max else template_HB * NA
final_opp_layer <- if(exists("leefgebied_opp")) leefgebied_opp else template_HB * NA
...
```{r export}
# ==============================================================================
# SCHONE BINAIR EXPORT ZONDER VALSE NULLEN (GEVLEKTE WITSNUITLIBEL)
# ==============================================================================
...
# 1. Maximale potentie
if (exists("leefgebied_max") && !all(is.na(terra::values(leefgebied_max, mat = FALSE)))) {
  potentie_export_rast <- terra::ifel(!is.na(leefgebied_max) & leefgebied_max > 0, 1, NA)
} else if (exists("final_max_layer") && !all(is.na(terra::values(final_max_layer, mat = FALSE)))) {
  potentie_export_rast <- terra::ifel(!is.na(final_max_layer) & final_max_layer > 0, 1, NA)
} else {
  potentie_export_rast <- terra::rast(template_HB, vals = NA)
}

# 2. Werkelijke oppervlakte
if (exists("leefgebied_opp") && !all(is.na(terra::values(leefgebied_opp, mat = FALSE)))) {
  werkelijk_export_rast <- terra::ifel(!is.na(leefgebied_opp) & leefgebied_opp > 0, 1, NA)
} else if (exists("final_opp_layer") && !all(is.na(terra::values(final_opp_layer, mat = FALSE)))) {
  werkelijk_export_rast <- terra::ifel(!is.na(final_opp_layer) & final_opp_layer > 0, 1, NA)
} else {
...
message("-> Alle GeoTIFF exports voor Gevlekte witsnuitlibel zijn succesvol weggeschreven!")
```
