...
template_HB_mask <- terra::rasterize(area_buffer_fix, id_raster_HB, field = 1)

# 3. Wiskundige index-conversie (lokale cel-index -> coordinaten -> globale cel-index)
lokale_ids  <- terra::cells(template_HB_mask)
coords      <- terra::xyFromCell(template_HB_mask, lokale_ids)
...
tabel_bebouw_HB <- tabel_vlaanderen_bebouw[CODE %in% bebouwings_lagen & cel_id %in% studiegebied_globale_ids]
...
rm(tabel_vlaanderen_bebouw, tabel_bebouw_HB, tabel_bebouw_som, tabel_bebouw_mapping, 
...
binnen_masker <- terra::rasterize(grens_merged, template_HB, field = 1, background = NA)
