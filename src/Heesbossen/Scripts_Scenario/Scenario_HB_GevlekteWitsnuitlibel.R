...
lokale_coords <- terra::xyFromCell(id_raster_HB, 1:ncell(id_raster_HB))
globale_ids   <- terra::cellFromXY(master_grid, lokale_coords)

vertaal_df <- data.table(
  lokale_id  = 1:ncell(id_raster_HB),
...
watervlakken_proj <- project(watervlakken_v, crs(template_HB))

area_buffer_clean  <- makeValid(area_buffer_fix) %>% aggregate()
watervlakken_clean <- makeValid(watervlakken_proj)
watervlakken_HB    <- crop(watervlakken_clean, area_buffer_clean)

# Fuzzy clustering op kleine wateren (<= 200m²)
watervlakken_buffered <- buffer(watervlakken_HB, width = 5)
...
if (nrow(kleine_clusters_v) > 0) {
  kleine_watervlakken_finaal <- crop(watervlakken_HB, kleine_clusters_v)
  if (nrow(kleine_watervlakken_finaal) > 0) {
    r_witsnuit_kleinwater        <- terra::rasterize(kleine_watervlakken_finaal, template_HB, field = 1, background = NA)
...
suppressWarnings(
  rm(watervlakken_v, watervlakken_proj, area_buffer_clean, watervlakken_clean, watervlakken_HB, 
...
# ==============================================================================
if (!is.null(r_witsnuit_waterbiotoop1) && !all(is.na(suppressWarnings(terra::minmax(r_witsnuit_waterbiotoop1))))) {
  ...
  if (!all(is.na(suppressWarnings(terra::minmax(cl_water_cand_max))))) {
    f_water_max <- terra::freq(cl_water_cand_max)
    f_water_max$Area_m2 <- f_water_max$count * 100
    
    valide_opp_ids_max <- f_water_max$value[f_water_max$Area_m2 <= drempel_water_max_m2]
    ...
    } else { r_water_cand_max <- template_HB * NA }
  } else { r_water_cand_max <- template_HB * NA }
  
  if (!all(is.na(suppressWarnings(terra::minmax(r_water_cand_max))))) {
    r_water_cl_max <- terra::patches(r_water_cand_max, directions = 8, zeroAsNA = TRUE)
    
    if (!all(is.na(suppressWarnings(terra::minmax(r_water_cl_max))))) {
      ...
      if (!is.null(watervlakken_poly) && nrow(watervlakken_poly) > 0) {
        ...
        if (nrow(goedgekeurde_clusters_poly) > 0) {
          gevlektewitsnuitlibel_voortplanting_max <- terra::rasterize(goedgekeurde_clusters_poly, template_HB, field = 1, background = NA)
          gevlektewitsnuitlibel_water_finaal_max  <- gevlektewitsnuitlibel_voortplanting_max
        } else {
          gevlektewitsnuitlibel_voortplanting_max <- template_HB * NA
          gevlektewitsnuitlibel_water_finaal_max  <- template_HB * NA
        }
      } else {
        gevlektewitsnuitlibel_voortplanting_max <- template_HB * NA
        gevlektewitsnuitlibel_water_finaal_max  <- template_HB * NA
...
# ==============================================================================
if (!is.null(r_witsnuit_waterbiotoop1_opp) && !all(is.na(suppressWarnings(terra::minmax(r_witsnuit_waterbiotoop1_opp))))) {
  ...
  if (!all(is.na(suppressWarnings(terra::minmax(cl_water_cand_opp))))) {
    ...
    } else { r_water_cand_opp <- template_HB * NA }
  } else { r_water_cand_opp <- template_HB * NA }
  
  if (!all(is.na(suppressWarnings(terra::minmax(gevlektewitsnuitlibel_voortplanting_max))))) {
    ...
    gevlektewitsnuitlibel_voortplanting_opp <- template_HB * NA
    gevlektewitsnuitlibel_water_finaal_opp <- template_HB * NA
  }
} else {
  gevlektewitsnuitlibel_voortplanting_opp <- template_HB * NA
  gevlektewitsnuitlibel_water_finaal_opp <- template_HB * NA
...
rm(r_witsnuit_waterbiotoop1, r_witsnuit_waterbiotoop1_opp)
gc()

# ==============================================================================
# WATER-FIRST LANDBIOTOOP FILTERING
# ==============================================================================
...
# --- STAP 1: RUIM LANDBIOTOOP FILTEREN ---
if (exists("land_ruim_max") && !all(is.na(suppressWarnings(terra::minmax(land_ruim_max))))) {
  res_ruim_max <- cluster_filter_compleet(
    masker     = land_ruim_max,
    opp_laag   = land_ruim_max,
    drempel_m2 = drempel_ruim_m2, 
    dist_m     = dist_ruim_m,     
    werkelijk  = FALSE
  )
  land_ruim_ok_max <- res_ruim_max$raster
} else { land_ruim_ok_max <- template_HB * NA }

if (exists("land_ruim_opp") && !all(is.na(suppressWarnings(terra::minmax(land_ruim_opp))))) {
...
} else { land_ruim_ok_opp <- template_HB * NA }

# --- STAP 2: SPOOR A - MAXIMALE POTENTIE ---
if (exists("gevlektewitsnuitlibel_voortplanting_max") && !all(is.na(suppressWarnings(terra::minmax(gevlektewitsnuitlibel_voortplanting_max))))) {
  ...
  if (exists("land_nabij_max") && !all(is.na(suppressWarnings(terra::minmax(land_nabij_max))))) {
    ...
    if (!all(is.na(suppressWarnings(terra::minmax(land_5ha_max))))) {
      r_struweel_max_clean <- fix_bos_raster(struweel1_max, template_HB)
      ...
      valide_ids_max <- df_check_max$Cluster_ID[df_check_max$Pct_Bos >= 30] 
      
      land_ok_max <- if(length(valide_ids_max) > 0) terra::mask(land_5ha_max, terra::ifel(cl_id_5ha_max %in% valide_ids_max, 1, NA)) else template_HB * NA
    } else { land_ok_max <- template_HB * NA }
    
    if (!all(is.na(suppressWarnings(terra::minmax(land_ok_max)))) && !all(is.na(suppressWarnings(terra::minmax(land_ruim_ok_max))))) {
  ...
  } else { gevlektewitsnuitlibel_land_finaal_max <- template_HB * NA }
} else { gevlektewitsnuitlibel_land_finaal_max <- template_HB * NA }

water_finaal_max  <- if(exists("gevlektewitsnuitlibel_water_finaal_max")) gevlektewitsnuitlibel_water_finaal_max else template_HB * NA
leefgebied_max    <- terra::cover(water_finaal_max, gevlektewitsnuitlibel_land_finaal_max)

# --- STAP 3: SPOOR B - WERKELIJKE OPPERVLAKTE ---
if (exists("gevlektewitsnuitlibel_voortplanting_opp") && !all(is.na(suppressWarnings(terra::minmax(gevlektewitsnuitlibel_voortplanting_opp))))) {
  ...
  if (exists("land_nabij_opp") && !all(is.na(suppressWarnings(terra::minmax(land_nabij_opp))))) {
    ...
    if (!all(is.na(suppressWarnings(terra::minmax(land_5ha_opp))))) {
      r_struweel_opp_clean <- fix_bos_raster(struweel1_opp, template_HB)
      ...
      valide_ids_opp <- df_check_opp$Cluster_ID[df_check_opp$Pct_Bos >= 30] 
      
      land_ok_opp <- if(length(valide_ids_opp) > 0) terra::mask(land_5ha_opp, terra::ifel(cl_id_5ha_opp %in% valide_ids_opp, 1, NA)) else template_HB * NA
    } else { land_ok_opp <- template_HB * NA }
    
    if (!all(is.na(suppressWarnings(terra::minmax(land_ok_opp)))) && !all(is.na(suppressWarnings(terra::minmax(land_ruim_ok_opp))))) {
  ...
  } else { gevlektewitsnuitlibel_land_finaal_opp <- template_HB * NA }
} else { gevlektewitsnuitlibel_land_finaal_opp <- template_HB * NA }

water_finaal_opp <- if(exists("gevlektewitsnuitlibel_voortplanting_opp")) gevlektewitsnuitlibel_voortplanting_opp else template_HB * NA
