...
hooggroen_sync    <- project(hooggroen_10m_raw, crs(template_HB)) %>% 
                     terra::resample(template_HB, method = "near")

# 2. TERRILS ISOLEREN (kg%)
tabel_terril <- tabel_vlaanderen[CODE %like% "^kg"]
tabel_terril_HB <- tabel_terril[cel_id %in% studiegebied_globale_ids]

tabel_terril_som <- tabel_terril_HB[, .(Oppervlakte = sum(BWK_FRAC, na.rm = TRUE)), by = .(cel_id)]

r_terril <- template_HB * NA
...
profiel_sync      <- terra::crop(raw_profiel, template_HB) %>% terra::resample(template_HB, method = "near")
drainage_sync     <- terra::crop(raw_drainage, template_HB) %>% terra::resample(template_HB, method = "near")
overstroming_sync <- terra::crop(raw_overstroming, template_HB) %>% terra::resample(template_HB, method = "near")
...
f_matrix <- terra::focalMat(template_HB, d = 20, type = "circle")
...
if(length(ids_ok_bin) > 0) {
  ...
  biotoop_fase7_bin <- template_HB * NA
  cl_id_fase7_bin   <- template_HB * NA
}
...
if(length(ids_ok_frac) > 0) {
  ...
  biotoop_fase7_frac <- template_HB * NA
  cl_id_fase7_frac   <- template_HB * NA
}
