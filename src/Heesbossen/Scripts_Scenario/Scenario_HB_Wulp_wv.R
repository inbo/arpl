...
template_HB <- terra::rasterize(area_shape_proj, id_raster_HB, field = 1, background = NA)
template_HB <- terra::ifel(!is.na(template_HB), 1, NA)

grens_web <- sf::st_as_sf(terra::project(area_shape, "EPSG:4326"))

rm(globale_id_raster, id_raster_HB_globale_values, id_raster_HB_masked)
gc()
...
if(!is.na(key_foerageer)) {
  foerageer_bwk_max <- lijst_matches[[key_foerageer]]
  foerageer_bwk_opp <- lijst_oppervlaktes[[key_foerageer]]
} else {
...
r_watervogelgebied_max <- terra::ifel(!is.na(template_HB), 1, NA)

# ==============================================================================
# STAP 2 - LEEFGEBIED CLUSTEREN (30 HA) EN BEGRENZEN (PARALLEL)
# ==============================================================================
message("-> 1. Open foerageerhabitat clusteren (50m fuzzy, drempel 30 ha)...")

wulp_leefgebied_max <- template_HB * NA
wulp_leefgebied_opp <- template_HB * NA

# --- SPOOR A: MAXIMALE POTENTIE (Binair) ---
if (exists("foerageer_bwk_max") && !is.null(foerageer_bwk_max) && !all(is.na(suppressWarnings(terra::minmax(foerageer_bwk_max))))) {
  ...
  if (!is.null(r_wulp_30ha_max) && !all(is.na(suppressWarnings(terra::minmax(r_wulp_30ha_max))))) {
    wulp_leefgebied_max <- terra::mask(r_wulp_30ha_max, r_watervogelgebied_max) %>% terra::crop(template_HB)
  }
}

# --- SPOOR B: WERKELIJKE OPPERVLAKTE ---
if (exists("foerageer_bwk_opp") && !is.null(foerageer_bwk_opp) && !all(is.na(suppressWarnings(terra::minmax(foerageer_bwk_opp))))) {
  ...
  if (!is.null(r_wulp_30ha_opp) && !all(is.na(suppressWarnings(terra::minmax(r_wulp_30ha_opp))))) {
    wulp_leefgebied_opp <- terra::mask(r_wulp_30ha_opp, r_watervogelgebied_max) %>% terra::crop(template_HB)
  }
}
