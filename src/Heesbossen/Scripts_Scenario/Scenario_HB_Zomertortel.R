...
# Gebied voorbereiden (Heesbossen met waterdichte 10km buffer)
...
id_raster_HB <- crop(master_grid, area_buffer_fix, snap = "out")

# FIX 2: Geef id_raster_HB een unieke opeenvolgende index voor lokale cellen (1..N)
values(id_raster_HB) <- 1:terra::ncell(id_raster_HB)

# FIX 3: Haal exact alle globale cel-IDs op voor de volledige omvang van id_raster_HB
globale_ids_vector <- terra::cells(master_grid, terra::ext(id_raster_HB))

# FIX 4: Bouw de vertaaltabel waterdicht op
vertaal_df <- data.table(
  lokale_id  = 1:terra::ncell(id_raster_HB),
...
template_HB <- terra::rasterize(area_buffer_fix, id_raster_HB, field = 1, background = NA)
...
tabel_bebouw_HB  <- tabel_vlaanderen[CODE %in% bebouwings_lagen & cel_id %in% studiegebied_globale_ids]
tabel_bebouw_som <- tabel_bebouw_HB[, .(Present = ifelse(sum(BWK_FRAC, na.rm = TRUE) > 0, 1, 0)), by = .(cel_id)]

r_bebouwing_raw <- id_raster_HB * NA
...
if (!is.null(r_bebouw_groot) && !terra::global(is.na(r_bebouw_groot), "sum")[[1]] == terra::ncell(r_bebouw_groot)) {
  ...
  masker_bebouw_100m_groot <- id_raster_HB * NA
}

rm(tabel_bebouw_HB, tabel_bebouw_som, tabel_bebouw_mapping, r_bebouwing_raw, res_bebouw_groot, r_bebouw_groot, tabel_vlaanderen)
gc()
