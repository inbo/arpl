...
SCENARIO_RDS_PAD <- here("data/input/Scenario_rds/HB_Scenario_bosbehoudss_ss31fix_tvg_vrij_2026.rds")

# Extraheer de naam van het scenario voor de logboeken en bestandsnamen
HUIDIG_SCENARIO  <- gsub("^HB_Scenario_|.rds$", "", basename(SCENARIO_RDS_PAD))
MAP_SCRIPTS      <- here("src/Heesbossen/Scripts_Scenario") # Map met de 60 scenario Rmd's

# AANGEPAST: Maak het pad dynamisch door HUIDIG_SCENARIO als submap toe te voegen
OUTPUT_DIR       <- here("data/output/Heesbossen/HTML_Rapporten_Scenario", HUIDIG_SCENARIO)
...
soorten_script_pad <- file.path(MAP_SCRIPTS, "Scenario_HB_Leefgebieden_Simpel.Rmd")
...
if (file.exists(soorten_script_pad)) {
  ...
  draai_leefgebied_scenario_model <- function(huidige_soort) {
    output_file_name <- paste0(HUIDIG_SCENARIO, "_HB_", huidige_soort, ".html")
    res_row <- data.frame(
      Item = huidige_soort,
      Type = "Simpele Soort",
      Status = "SUCCES",
