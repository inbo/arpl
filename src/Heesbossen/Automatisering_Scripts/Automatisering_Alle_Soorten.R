...
MAP_WAARNEMINGEN   <- here("data/input/Waarnemingen_Soorten/Heesbossen")
...
# ------------------------------------------------------------------------------
alle_tv_scripts <- list.files(path = MAP_SCRIPTS_HB, pattern = "\\.Rmd$", full.names = TRUE)
soorten_script_tv_pad <- file.path(MAP_SCRIPTS_HB, "HB_Leefgebieden_Simpel.Rmd")
unieke_tv_scripts <- setdiff(alle_tv_scripts, soorten_script_tv_pad)

message("=> Gedetecteerde unieke HB-scripts om te knitten (Aantal: ", length(unieke_tv_scripts), "):")
...
message("\n=> Aantal geselecteerde simpele soorten voor HB-run (met waarnemingenfile): ", length(soorten_lijst))
