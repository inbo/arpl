# ==============================================================================
# MASTER AUTOMATISERINGSSCRIPT VOOR REGULIERE HB-SCRIPTS (KNITTEN)
...
# ------------------------------------------------------------------------------
MAP_SCRIPTS_HB   <- here("src/Heesbossen/Scripts_HB")
OUTPUT_DIR       <- here("data/output/Heesbossen/HTML_Rapporten_Soorten")
...
message("==================================================")
message(" START MASTER RUN: NORMALE HB SCRIPTS KNITTEN     ")
message("==================================================")
...
# ------------------------------------------------------------------------------
# 1. DETECTEER EN KNIT DE UNIEKE HB-SCRIPTS
# ------------------------------------------------------------------------------
alle_hb_scripts <- list.files(path = MAP_SCRIPTS_HB, pattern = "\\.Rmd$", full.names = TRUE)
soorten_script_hb_pad <- file.path(MAP_SCRIPTS_HB, "HB_Leefgebieden_Simpel.Rmd")
unieke_hb_scripts <- setdiff(alle_hb_scripts, soorten_script_hb_pad)

message("=> Gedetecteerde unieke HB-scripts om te knitten (Aantal: ", length(unieke_hb_scripts), "):")
print(basename(unieke_hb_scripts))
message("--------------------------------------------------")

for(script in unieke_hb_scripts) {
  ...
  if (!moet_knitten(output_html, OUTPUT_DIR)) {
    ...
    algemeen_logboek[[bestandsnaam]] <- data.frame(
      Item = bestandsnaam, Type = "Uniek HB Script", Status = "GEKOZEN_OVERSLAAN", Fout = "Reeds vandaag geknit", stringsAsFactors = FALSE
    )
  ...
  tryCatch({
    ...
    algemeen_logboek[[bestandsnaam]] <- data.frame(
      Item = bestandsnaam, Type = "Uniek HB Script", Status = "SUCCES", Fout = "Geen", stringsAsFactors = FALSE
    )
  }, error = function(e) {
    message("❌ FOUT bij: ", bestandsnaam)
    algemeen_logboek[[bestandsnaam]] <- data.frame(
      Item = bestandsnaam, Type = "Uniek HB Script", Status = "CRASH", Fout = e$message, stringsAsFactors = FALSE
    )
  })
}
...
message("\n=> Aantal geselecteerde simpele soorten voor HB-run: ", length(soorten_lijst))


# ------------------------------------------------------------------------------
# 3. RUN HET 'SIMPEL_HB' SCRIPT PER SOORT (Knitten met parameter)
# ------------------------------------------------------------------------------
draai_leefgebied_model <- function(huidige_soort, script_pad) {
  output_file_name <- paste0("HB_", huidige_soort, ".html")
...
if (length(soorten_lijst) > 0) {
  soorten_logboek <- purrr::map_dfr(
    soorten_lijst, 
    ~draai_leefgebied_model(huidige_soort = .x, script_pad = soorten_script_hb_pad)
  )
...
# ------------------------------------------------------------------------------
log_file_path <- file.path(OUTPUT_DIR, paste0("Logboek_HB_NormaalRun_", format(Sys.time(), "%Y%m%d_%H%M"), ".csv"))
...
message("\n==================================================")
message(" MASTER RUN HB NORMAAL COMPLEET!                  ")
