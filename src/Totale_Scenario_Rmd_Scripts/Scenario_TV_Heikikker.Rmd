...
# Hoofdletter- en sleutel-veilige toewijzing
...
if (file.exists(watervlakken_path)) {
  ...
  message("⚠️ Geen watervlakken shapefile gevonden; waterbiotoop valt terug op BWK.")
  r_heikikker_waterbiotoop1     <- waterbiotoop_bwk_max
  r_heikikker_waterbiotoop1_opp <- waterbiotoop_bwk_opp
...
potentie_export_rast     <- if(exists("heikikker_leefgebied_max") && !all(is.na(terra::values(heikikker_leefgebied_max, mat=FALSE)))) terra::ifel(!is.na(heikikker_leefgebied_max) & heikikker_leefgebied_max > 0, 1, NA) else terra::rast(template_TV, vals = NA)
werkelijk_export_rast    <- if(exists("heikikker_leefgebied_opp") && !all(is.na(terra::values(heikikker_leefgebied_opp, mat=FALSE)))) terra::ifel(!is.na(heikikker_leefgebied_opp) & heikikker_leefgebied_opp > 0, 1, NA) else terra::rast(template_TV, vals = NA)
arpl_export_rast         <- if(exists("heikikker_arpl_opp") && !all(is.na(terra::values(heikikker_arpl_opp, mat=FALSE)))) terra::ifel(heikikker_arpl_opp > 0, 1, NA) else terra::rast(template_TV, vals = NA)
verspreiding_export_rast <- if(exists("heikikker_verspreiding_bin") && !all(is.na(terra::values(heikikker_verspreiding_bin, mat=FALSE)))) terra::ifel(heikikker_verspreiding_bin > 0, 1, NA) else terra::rast(template_TV, vals = NA)
...
```{r export}
# ==============================================================================
# 2. SCHONE BINAIR EXPORT ZONDER VALSE NULLEN (SCENARIO HEIKIKKER)
# ==============================================================================
...
message("🏁 SCENARIO EXPORT VOLLEDIG AFGEROND VOOR HEIKIKKER!")
```
