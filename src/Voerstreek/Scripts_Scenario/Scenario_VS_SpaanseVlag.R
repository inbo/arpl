...
if(length(goedgekeurde_rand_ids_max) > 0) {
  ...
  spaansevlag_bosrand2_max <- id_raster_VS * NA
}
...
if(length(goedgekeurde_rand_ids_opp) > 0) {
  ...
  spaansevlag_bosrand2_opp <- id_raster_VS * NA
}
...
final_max <- spaansevlag_leefgebied_finaal_max %>% terra::crop(template_VS)
final_opp <- spaansevlag_leefgebied_finaal_opp %>% terra::crop(template_VS)
cl_max    <- final_max
bwk_opp   <- final_opp
