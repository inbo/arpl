...
# ==============================================================================
# STAP 1: VOORTPLANTINGSGEBIED (MINIMAAL 50 HA, 100M CLUSTERING - PARALLEL)
# ==============================================================================
...
# ==============================================================================
# STAP 2: OPEN FOERAGEERGEBIED (MINIMAAL 50 HA, 500M CLUSTERING - PARALLEL)
# ==============================================================================
...
# ==============================================================================
# STAP 3: OPTIMAAL FOERAGEERCOMPLEX (MINIMAAL 250 HA, 500M - PARALLEL)
# ==============================================================================
...
# ==============================================================================
# STAP 4: ACTIERADIUS CHECK (5 KM) EN FINALE BROEDBOS SELECTIE (PARALLEL)
# ==============================================================================
message("-> Actieradiuskoppeling (5000m) berekenen tussen broedbos en foerageercomplex...")

# --- SPOOR A: MAX ---
if (!all(is.na(terra::values(r_foerageer2_max, mat=FALSE)))) {
  buffer_5000m_foer_max <- terra::buffer(r_foerageer2_max, width = 5000)
  wespendief_leefgebied_max <- terra::mask(r_wespendief_bos_max, buffer_5000m_foer_max) %>% terra::crop(template_VS)
  suppressWarnings(rm(buffer_5000m_foer_max))
} else {
  wespendief_leefgebied_max <- template_VS * NA
}

# --- SPOOR B: OPP (Parallel & Autonoom) ---
if (!all(is.na(terra::values(r_foerageer2_opp, mat=FALSE)))) {
  r_binair_foer2_opp <- terra::ifel(!is.na(r_foerageer2_opp) & r_foerageer2_opp > 0, 1, NA)
  buffer_5000m_foer_opp <- terra::buffer(r_binair_foer2_opp, width = 5000)
  
  wespendief_leefgebied_opp <- terra::mask(r_wespendief_bos_opp, buffer_5000m_foer_opp) %>% terra::crop(template_VS)
  suppressWarnings(rm(r_binair_foer2_opp, buffer_5000m_foer_opp))
} else {
  wespendief_leefgebied_opp <- template_VS * NA
}

cat("Definitief leefgebied Wespendief MAX (ha):", round(calc_ha_exact(wespendief_leefgebied_max), 2), "\n")
cat("Definitief leefgebied Wespendief OPP (ha):", round(calc_ha_exact(wespendief_leefgebied_opp), 2), "\n")
