...
vroedmeesterpad_arpl_opp         <- terra::rast(template_VS, vals = NA)
vroedmeesterpad_verspreiding_bin <- terra::rast(template_VS, vals = NA) 
...
if (!all(is.na(terra::values(vroedmeesterpad_leefgebied_max, mat=FALSE)))) {
  ...
  colnames(zonal_in_tv_max) <- c("ID", "In_VS")
  
  ids_pot_bin  <- zonal_in_tv_max$ID[!is.na(zonal_in_tv_max$In_VS) & zonal_in_tv_max$In_VS > 0]
  ids_alle_max <- unique(na.omit(terra::values(r_opp_net_ids_max, mat = FALSE)))
  ids_pot_buit <- setdiff(ids_alle_max, ids_pot_bin)
  
  r_max_tv     <- terra::crop(vroedmeesterpad_leefgebied_max, template_VS)
  r_net_max_tv <- terra::crop(r_opp_net_ids_max, template_VS)
...
if (!all(is.na(terra::values(vroedmeesterpad_leefgebied_opp, mat=FALSE)))) {
  ...
  colnames(zonal_in_tv_opp) <- c("ID", "In_VS")
  
  ids_werk_bin  <- zonal_in_tv_opp$ID[!is.na(zonal_in_tv_opp$In_VS) & zonal_in_tv_opp$In_VS > 0]
  ids_alle_opp  <- unique(na.omit(terra::values(r_opp_net_ids_opp, mat = FALSE)))
  ids_werk_buit <- setdiff(ids_alle_opp, ids_werk_bin)
  
  r_opp_tv     <- terra::crop(vroedmeesterpad_leefgebied_opp, template_VS)
  r_net_opp_tv <- terra::crop(r_opp_net_ids_opp, template_VS)
  ...
  if (has_observations && nrow(df_points_sf) > 0) {
    ...
    if(length(ids_arpl) > 0) {
      ...
      vroedmeesterpad_arpl_opp <- terra::mask(r_opp_tv, terra::crop(terra::ifel(m_arpl == 1, 1, NA), template_VS))
    ...
    if(length(ids_verspr) > 0) {
      ...
      clusters_actueel_tv <- terra::crop(terra::mask(r_opp_tv, terra::ifel(m_verspr == 1, 1, NA)), template_VS)
...
if (exists("r_net_max_tv") && !all(is.na(terra::values(r_net_max_tv, mat=FALSE)))) {
  r_net_ids_sync <- terra::resample(r_net_max_tv, template_VS, method = "near")
