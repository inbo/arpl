...
message("==================================================")
message(" START MASTER RUN: NORMALE VS SCRIPTS KNITTEN     ")
message("==================================================")
...
# ------------------------------------------------------------------------------
alle_vs_scripts <- list.files(path = MAP_SCRIPTS_VS, pattern = "\\.Rmd$", full.names = TRUE)
soorten_script_vs_pad <- file.path(MAP_SCRIPTS_VS, "VS_Leefgebieden_Simpel.Rmd")
unieke_vs_scripts <- setdiff(alle_vs_scripts, soorten_script_vs_pad)

message("=> Gedetecteerde unieke VS-scripts om te knitten (Aantal: ", length(unieke_vs_scripts), "):")
print(basename(unieke_vs_scripts))
message("--------------------------------------------------")

for(script in unieke_vs_scripts) {
...
message("\n=> Aantal geselecteerde simpele soorten voor VS-run: ", length(soorten_lijst))
...
if (length(soorten_lijst) > 0) {
  soorten_logboek <- purrr::map_dfr(
    soorten_lijst, 
    ~draai_leefgebied_model(huidige_soort = .x, script_pad = soorten_script_vs_pad)
  )
