...
```{r export}
# ==============================================================================
# SCHONE BINAIR EXPORT ZONDER VALSE NULLEN (BOOMPIEPER - TURNHOUTS VENNEGEBIED)
# ==============================================================================
message(paste("-> Start geformatteerde binaire export met nul-opvulling voor:", soort))
...
# 1. Maximale potentie
if (exists("leefgebied_finaal_max_binair") && !all(is.na(terra::values(leefgebied_finaal_max_binair, mat = FALSE)))) {
...
if (exists("leefgebied_finaal_opp") && !all(is.na(terra::values(leefgebied_finaal_opp, mat = FALSE)))) {
...
if (exists("boompieper_arpl_opp") && !all(is.na(terra::values(boompieper_arpl_opp, mat = FALSE)))) {
  arpl_export_rast <- terra::ifel(!is.na(boompieper_arpl_opp) & boompieper_arpl_opp > 0, 1, NA)
} else {
  arpl_export_rast <- terra::rast(template_TV, vals = NA)
}

# 4. Actuele verspreiding
if (exists("boompieper_verspreiding_bin") && !all(is.na(terra::values(boompieper_verspreiding_bin, mat = FALSE)))) {
  verspreiding_export_rast <- terra::ifel(!is.na(boompieper_verspreiding_bin) & boompieper_verspreiding_bin > 0, 1, NA)
} else {
