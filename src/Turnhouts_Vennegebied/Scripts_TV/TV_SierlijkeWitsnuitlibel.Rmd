...
# ==============================================================================
# STAP 8: INTERACTIEVE KAART (LEAFLET - MET EXPLICIETE GRENSAANDUIDING)
# ==============================================================================

# 1. HERSTEL STUDIEGEBIED GRENS (ONAFHANKELIJK VAN OVERIGE LAGEN)
grens_web <- sf::st_as_sf(terra::project(area_shape, "EPSG:4326"))
bbox      <- sf::st_bbox(grens_web)

# 2. LAND- EN TOTAAL LEEFGEBIED VOORBEREIDEN
if (exists("cl_id_opp") && !all(is.na(terra::values(cl_id_opp, mat=FALSE)))) {
  
  message("-> Leaflet-kaart opbouwen voor Sierlijke witsnuitlibel...")
    ...
    cl_poly_final <- cl_poly_opp %>%
      ...
      mutate(
        ...
        Popup = paste0(
          ...
          "<strong>Soort:</strong> Sierlijke witsnuitlibel<br><hr>",
          ...
          "<strong>Netto Biotoop binnen 800m:</strong> <b style='color:#008800;'>", Werkelijke_Ha, " ha</b><br>",
          "</div>"
        )
      ) %>%
...
if(has_land_layer && nrow(cl_poly_final) > 0) {
  map <- map %>% addPolygons(
    ...
    group = "Werkelijk Leefgebied (Max 800m)"
  )
}

# LAAG 2: FELBLAUWE VENNEN / WATERBIOTOOP (Voorgrond)
if(has_water_layer) {
  map <- map %>% addPolygons(
    data = water_poly,
    color = "#0033aa",
...
# LAAG 5: STUDIEGEBIED GRENS (ZWARTE LIJN BOVENOP)
map %>%
  addPolygons(
    data = grens_web, 
    fill = FALSE, 
  ...
  addLegend(
    ...
    labels = c("Landbiotoop (Max 800m)", "Geschikte Vennen/Poelen", "Waarnemingen"),
    title = "Sierlijke witsnuitlibel",
    position = "bottomright"
  ) %>%
  addLayersControl(
    baseGroups = c("Satelliet", "OSM (Kaart)"),
    overlayGroups = c(
      "Werkelijk Leefgebied (Max 800m)", 
...
# ==============================================================================
# SCHONE BINAIR EXPORT ZONDER VALSE NULLEN (SIERLIJKE WITSNUITLIBEL - TURNHOUTS VENNEGEBIED)
# ==============================================================================
...
# 1. Maximale potentie
if (exists("finaal_max_binair") && !all(is.na(terra::values(finaal_max_binair, mat = FALSE)))) {
  potentie_export_rast <- terra::ifel(!is.na(finaal_max_binair) & finaal_max_binair > 0, 1, NA)
} else if (exists("sierlijkewitsnuitlibel_finaal_max") && !all(is.na(terra::values(sierlijkewitsnuitlibel_finaal_max, mat = FALSE)))) {
  potentie_export_rast <- terra::ifel(!is.na(sierlijkewitsnuitlibel_finaal_max) & sierlijkewitsnuitlibel_finaal_max > 0, 1, NA)
} else {
...
} else if (exists("sierlijkewitsnuitlibel_finaal_opp") && !all(is.na(terra::values(sierlijkewitsnuitlibel_finaal_opp, mat = FALSE)))) {
  werkelijk_export_rast <- terra::ifel(!is.na(sierlijkewitsnuitlibel_finaal_opp) & sierlijkewitsnuitlibel_finaal_opp > 0, 1, NA)
} else {
...
if (exists("sierlijkewitsnuitlibel_arpl_opp") && !all(is.na(terra::values(sierlijkewitsnuitlibel_arpl_opp, mat = FALSE)))) {
  arpl_export_rast <- terra::ifel(!is.na(sierlijkewitsnuitlibel_arpl_opp) & sierlijkewitsnuitlibel_arpl_opp > 0, 1, NA)
} else {
...
if (exists("sierlijkewitsnuitlibel_verspr_opp") && !all(is.na(terra::values(sierlijkewitsnuitlibel_verspr_opp, mat = FALSE)))) {
  verspreiding_export_rast <- terra::ifel(!is.na(sierlijkewitsnuitlibel_verspr_opp) & sierlijkewitsnuitlibel_verspr_opp > 0, 1, NA)
} else {
