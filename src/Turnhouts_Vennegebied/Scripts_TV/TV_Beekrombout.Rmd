...
```{r export}
# ==============================================================================
# SCHONE BINAIR EXPORT ZONDER VALSE NULLEN (BEEKROMBOUT - TURNHOUTS VENNEGEBIED)
# ==============================================================================
...
# Helper-functie: Maak binair (1 = aanwezig/geschikt, NA = afwezig)
to_binary_raster <- function(r) {
  if (is.null(r) || all(is.na(terra::values(r, mat = FALSE)))) {
    return(template_TV * NA)
  }
  return(terra::ifel(!is.na(r) & r > 0, 1, NA))
}

# Converteer alle te exporteren lagen naar een strak binair raster (1 / NA)
export_config <- list(
  list(rast = to_binary_raster(final_max),               naam = "Maximale_Potentie",        folder = folders$potentie),
  list(rast = to_binary_raster(final_opp),               naam = "Werkelijke_Oppervlaktes", folder = folders$werkelijk), 
  list(rast = to_binary_raster(beekrombout_arpl_opp),   naam = "ARPL",                    folder = folders$arpl),
  list(rast = to_binary_raster(beekrombout_verspr_opp), naam = "Actuele_Verspreiding",    folder = folders$verspreiding)
)
...
for(item in export_config) {
  ...
  # Lijn uit met het MasterGrid (behoudt NA voor cellen buiten het studiegebied)
  export_rast <- terra::extend(export_rast, master_grid, fill = NA)
  
  # Schrijf naar schijf met expliciete NAflag = 255 voor transparantie in GIS
  ...
  message(paste("    [OK] Binaire kaart geëxporteerd:", basename(file_path)))
}

suppressWarnings(
  rm(export_config, export_rast)
)
gc()

message("🏁 SCRIPT EN EXPORT VOLLEDIG EN SUCCESVOL AFGEROND VOOR BEEKROMBOUT!")
```