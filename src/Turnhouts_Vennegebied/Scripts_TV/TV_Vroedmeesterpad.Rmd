...
```{r export}
message("-> Start geformatteerde export met nul-opvulling voor vroedmeesterpad...")
...
if(exists("vroedmeesterpad_arpl_opp") && !all(is.na(terra::values(vroedmeesterpad_arpl_opp, mat=FALSE)))) {
  arpl_export_rast <- terra::ifel(vroedmeesterpad_arpl_opp > 0, 1, NA)
} else {
  arpl_export_rast <- terra::rast(template_TV, vals = NA)
}

if(!all(is.na(terra::values(vroedmeesterpad_verspreiding_bin, mat=FALSE)))) {
  verspreiding_export_rast <- terra::ifel(vroedmeesterpad_verspreiding_bin > 0, 1, NA)
} else {
  verspreiding_export_rast <- terra::rast(template_TV, vals = NA)
}

export_config <- list(
  list(rast = vroedmeesterpad_leefgebied_max, naam = "Maximale_Potentie",        folder = folders$potentie,     type = "INT1U"),
