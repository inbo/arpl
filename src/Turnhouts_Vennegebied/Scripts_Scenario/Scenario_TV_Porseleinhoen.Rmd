...
rm(resultaten_gegroepeerd, df_nieuw, lijst_oppervlaktes, tabel_gefilterd, tabel_TV, tabel_cel_som)
gc()
```

```{r Biotoopverwerking en Filters}
# ==============================================================================
# STAP 1 T/M 5: BIOTOOPCOMBINATIE, STOORLAGEN, DRAINAGE & CLUSTERING (ENKEL WERKLIJK)
# ==============================================================================
message("=== STAP 1: Biotopen combineren en uitsluitingszones toepassen ===")

# 1. COMBINEER GRASLAND EN MOERAS FRACTIES
...
biotoop_opp_tot  <- terra::clamp(biotoop_opp_som, upper = 1.0)
biotoop_opp      <- terra::ifel(biotoop_opp_tot > 0, biotoop_opp_tot, NA)

# 2. HOOGGROEN EN TERRILS INLEZEN
...
if(nrow(tabel_terril_TV) > 0) {
  ...
  if(nrow(tabel_terril_mapping) > 0) r_terril[tabel_terril_mapping$lokale_id] <- 1
}
...
# ==============================================================================
# STAP 2: BEPERKEN TOT NATTE GRONDEN VIA DRAINAGEKAART
# ==============================================================================
...
# ==============================================================================
# STAP 3: EERSTE CLUSTERING (3 HA / 50M)
# ==============================================================================
message("=== STAP 3: Eerste clustering en filtering op 3 hectare ===")

drempel_3ha_m2 <- 3 * 10000
r_bin_3ha <- terra::ifel(!is.na(porseleinhoen_geschikt3_opp) & porseleinhoen_geschikt3_opp > 0, 1, NA)
...
# ==============================================================================
# STAP 4: OPEN ZICHT FILTER (20 ARE BOSDREMPEL / 100M BUFFER)
# ==============================================================================
...
if(length(clusters_20are_ids) > 0) {
  ...
  porseleinhoen_geschikt5_opp <- cluster_filter_compleet(r_bin_fase5, opp_laag = biotoop_fase5_kaal_frac, 30000, 50, TRUE)$raster
} else {
  porseleinhoen_geschikt5_opp <- porseleinhoen_geschikt4_opp
}

# ==============================================================================
# STAP 5: FINALE CLUSTERING & EINDDREMPEL (10 HA)
# ==============================================================================
message("=== FINALE STAP: Filtering van het leefgebiedcomplex op 10 hectare ===")

drempel_10ha_m2 <- 10 * 10000
r_bin_10ha <- terra::ifel(!is.na(porseleinhoen_geschikt5_opp) & porseleinhoen_geschikt5_opp > 0, 1, NA)
...
cat("\n--- AUDIT: SCENARIO LEEFGEBIED PORSELEINHOEN (WERKELIJKE OPPERVLAKTE) ---\n")
ha_finaal_werk <- calc_ha_exact(finaal_leefgebied_opp)
cat("Werkelijke oppervlakte leefgebied Porseleinhoen in scenario (>= 10 ha):", round(ha_finaal_werk, 2), "ha\n")

suppressWarnings(rm(biotoop_opp_clean_gras, biotoop_opp_clean_moer, biotoop_opp_som, biotoop_opp_tot, biotoop_opp,
   groenkaart, groenkaart_TV, hooggroen_aligned, r_terril, stoorlaag, tabel_terril, tabel_terril_TV,
   porseleinhoen_geschikt2_opp, r_drain_raw, r_drain_local, masker_drainage, mal_drainage, drain_cats,
   porseleinhoen_geschikt3_opp, r_bin_3ha, cluster_3ha_opp, porseleinhoen_geschikt4_opp, r_groen_bin, w_matrix, 
   groen_buren, r_groen_clean, groen_patches, groen_freq, clusters_20are_ids, dist_groen_clusters, 
   masker_groen_100m_cluster, porseleinhoen_geschikt5_opp, r_bin_10ha, finaal_10ha_opp, tabel_vlaanderen))
gc()
```
...
if (exists("finaal_leefgebied_opp") && !all(is.na(terra::values(finaal_leefgebied_opp, mat = FALSE)))) {
  werkelijk_scenario_rast <- terra::ifel(!is.na(finaal_leefgebied_opp) & finaal_leefgebied_opp > 0, 1, NA)
} else {
