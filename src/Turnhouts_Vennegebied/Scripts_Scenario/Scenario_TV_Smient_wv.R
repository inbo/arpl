...
water_bwk_max    <- lijst_matches[["foerageer_water"]]
water_bwk_opp    <- lijst_oppervlaktes[["foerageer_water"]]

grasland_bwk_max <- lijst_matches[["foerageer_grasland"]]
grasland_bwk_opp <- lijst_oppervlaktes[["foerageer_grasland"]]
...
# ==============================================================================
# STAP 2: EXPERT-LOGICA SMIENT
# 1. Graslanden (h-reeks) grenzend aan/nabij water = DIRECT GOEDGEKEURD
# 2. Losstaande plassen zonder grasland = WEL 1 ha MINIMALE OPPERVLAKTE
# ==============================================================================
message("-> Waterlopen en INBO Watervlakkenkaart (2024) inlezen...")
...
gewenste_water_codes <- c("Mlz", "O1brak", "O1o", "O2zout", "Rg")
...
# 1. BUNDEL ALLE WATERHABITATS (BWK Water/Vennen + Waterlopen + Watervlakken)
water_totaal_max <- (!is.na(water_bwk_max) & water_bwk_max == 1) | 
                    (r_waterlopen == 1) | 
                    (!is.na(r_watervlakken_ok) & r_watervlakken_ok == 1)
water_totaal_max <- terra::ifel(water_totaal_max == 1, 1, NA)

# 2. GRASLAND-KOPPELING: Graslanden (h-reeks) binnen 100m van water
if (!all(is.na(terra::values(water_totaal_max, mat=FALSE))) && !is.null(grasland_bwk_max)) {
  water_buffer_100m      <- terra::buffer(water_totaal_max, width = 100)
  grasland_gekoppeld_max <- terra::mask(grasland_bwk_max, water_buffer_100m)
  grasland_gekoppeld_opp <- terra::mask(grasland_bwk_opp, water_buffer_100m)
} else {
  grasland_gekoppeld_max <- template_TV * NA
  grasland_gekoppeld_opp <- template_TV * NA
}

# 3. WATER-GEBIEDEN DIE NIET AAN GRASLAND GRENZEN: FILTEREN OP MINSTENS 1 HA
if (!all(is.na(terra::values(grasland_gekoppeld_max, mat=FALSE)))) {
  grasland_buffer <- terra::buffer(grasland_gekoppeld_max, width = 20)
  water_aan_grasland <- terra::mask(water_totaal_max, grasland_buffer)
  water_losstaand    <- terra::mask(water_totaal_max, grasland_buffer, inverse = TRUE)
} else {
  water_aan_grasland <- template_TV * NA
  water_losstaand    <- water_totaal_max
}

# Pas de 1 ha ondergrens uitsluitend toe op de losstaande plassen
drempel_water_m2 <- oppervlakte_ha * 10000 # 1 ha = 10.000 m²
water_losstaand_gefilterd <- cluster_filter_compleet(water_losstaand, water_losstaand, drempel_water_m2, 50, FALSE)$raster

# 4. SAMENVOEGEN VAN HET COMPLETE HABITAT
smient_totaal_max <- terra::cover(water_aan_grasland, water_losstaand_gefilterd)
smient_totaal_max <- terra::cover(smient_totaal_max, grasland_gekoppeld_max)

smient_leefgebied_max <- terra::mask(smient_totaal_max, r_watervogelgebied_max) %>% terra::crop(template_TV)

# Werkelijke oppervlaktespoor
r_water_opp <- terra::cover(water_bwk_opp, terra::ifel(r_waterlopen == 1, 1, NA))
r_water_opp <- terra::cover(r_water_opp, terra::ifel(!is.na(r_watervlakken_ok), 1, NA))
r_water_opp <- terra::cover(r_water_opp, grasland_bwk_opp)

smient_leefgebied_opp <- terra::mask(r_water_opp, smient_leefgebied_max) %>% terra::crop(template_TV)

message("--- SMIENT MODEL SUCCESVOL AFGEROND CONFORM EXPERT-FEEDBACK ---")
cat("Totaal stabiel leefgebied Smient (ha):", round(calc_ha_exact(smient_leefgebied_opp), 2), "\n")

rm(r_huetzon_raw, r_huetzon, huetzon_cats, ids_waterlopen_ok, r_waterlopen, r_watervlakken_raw, r_watervlakken, r_watervlakken_ok,
   water_totaal_max, grasland_gekoppeld_max, grasland_gekoppeld_opp, water_aan_grasland, water_losstaand, water_losstaand_gefilterd, smient_totaal_max, r_watervogelgebied_max)
gc()
