...
if (!is.null(opp_bron) && !all(is.na(terra::values(opp_bron, mat = FALSE)))) {
  r_binair <- terra::ifel(!is.na(opp_bron) & opp_bron > 0, 1, NA)
  ...
  eikelmuis_leefgebied_opp <- terra::mask(opp_bron, res_opp$raster) %>% terra::crop(template_TV)
} else {
  eikelmuis_leefgebied_opp <- template_TV * NA
}

cat("\n--- AUDIT: SCENARIO LEEFGEBIED EIKELMUIS (WERKELIJKE OPPERVLAKTE) ---\n")
ha_finaal_werk <- calc_ha_exact(eikelmuis_leefgebied_opp)
cat("Werkelijke oppervlakte leefgebied Eikelmuis in scenario:", round(ha_finaal_werk, 2), "ha\n")
...
if (exists("eikelmuis_leefgebied_opp") && !all(is.na(terra::values(eikelmuis_leefgebied_opp, mat = FALSE)))) {
  werkelijk_scenario_rast <- terra::ifel(!is.na(eikelmuis_leefgebied_opp) & eikelmuis_leefgebied_opp > 0, 1, NA)
} else {
