...
if (exists("heide_max") && !all(is.na(terra::values(heide_max, mat=FALSE)))) {
  ...
  r_heide_finaal_max <- template_TV * NA
  r_heide_finaal_opp <- template_TV * NA
}

message("-> 3. Samenvoegen van Open Cultuurlandschap en Heide...")
regenwulp_leefgebied_max <- terra::cover(r_open_land_finaal_max, r_heide_finaal_max) %>% terra::crop(template_TV)
regenwulp_leefgebied_opp <- terra::cover(r_open_land_finaal_opp, r_heide_finaal_opp) %>% terra::crop(template_TV)
