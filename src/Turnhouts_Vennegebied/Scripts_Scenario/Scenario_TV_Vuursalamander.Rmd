---
title: "Scenario Leefgebiedenscript: Vuursalamander"
...
```{r Cluster}
# ==============================================================================
# CLUSTERING EN OPPERVLAKTEFILTERING (Spoor B)
# ==============================================================================
message("-> Ruimtelijke clustering berekenen op basis van reële oppervlakte-fracties...")
...
if (!is.null(bwk_opp) && !all(is.na(terra::values(bwk_opp, mat = FALSE)))) {
  ...
  vuursalamander_leefgebied_opp <- terra::mask(bwk_opp, res_opp$raster) %>% terra::crop(template_TV)
} else {
  vuursalamander_leefgebied_opp <- template_TV * NA
}

cat("\n--- AUDIT: SCENARIO LEEFGEBIED VUURSALAMANDER (WERKELIJKE OPPERVLAKTE) ---\n")
ha_finaal_werk <- calc_ha_exact(vuursalamander_leefgebied_opp)
cat("Werkelijke oppervlakte leefgebied Vuursalamander in scenario:", round(ha_finaal_werk, 2), "ha\n")

suppressWarnings(rm(r_binair, res_opp))
gc()
```
...
if (exists("vuursalamander_leefgebied_opp") && !all(is.na(terra::values(vuursalamander_leefgebied_opp, mat = FALSE)))) {
  werkelijk_scenario_rast <- terra::ifel(!is.na(vuursalamander_leefgebied_opp) & vuursalamander_leefgebied_opp > 0, 1, NA)
} else {
