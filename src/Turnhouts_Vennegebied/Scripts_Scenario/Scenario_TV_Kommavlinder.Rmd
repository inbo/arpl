...
```{r ku koppeling en cluster filtering}
# ==============================================================================
# 4. KOPPELING KU EN BWK & CLUSTERFILTERING (ENKEL WERKLIJKE OPPERVLAKTES)
# ==============================================================================
message("-> Ku-cellen filteren op basis van afstand tot BWK-cellen (50m)...")

r_binair_bwk <- terra::ifel(!is.na(bwk_opp) & bwk_opp > 0, 1, NA)
bwk_buffer_50m <- terra::distance(r_binair_bwk) <= 50

# Filter KU-cellen op reële oppervlaktes
ku_gefilterd_opp <- terra::mask(ku_opp, bwk_buffer_50m == 1)

# Samenvoegen van fracties (NA omzetten naar 0)
bwk_clean <- terra::ifel(is.na(bwk_opp), 0, bwk_opp)
ku_clean  <- terra::ifel(is.na(ku_gefilterd_opp), 0, ku_gefilterd_opp)

som_fracties   <- bwk_clean + ku_clean
som_gemedieerd <- terra::clamp(som_fracties, upper = 1.0)

# Alleen cellen met werkelijk habitat behouden (> 0)
kommavlinder_biotoop_gecombineerd_opp <- terra::ifel(som_gemedieerd > 0, som_gemedieerd, NA)
r_binair_gecombineerd <- terra::ifel(!is.na(kommavlinder_biotoop_gecombineerd_opp), 1, NA)

# ==============================================================================
# 5. CLUSTEREN EN MINIMUM OPPERVLAKTE FILTERING (1 ha / 10m)
# ==============================================================================
message("-> Gecombineerd biotoop filteren op minimumoppervlakte (1 ha / 10000 m²)...")

drempel_m2 <- oppervlakte_ha * 10000
zoekafstand_m <- 10

biotoop_opp_lijst <- cluster_filter_compleet(
  masker     = r_binair_gecombineerd,
...
cat("\n--- AUDIT: SCENARIO BIOTOOP KOMMAVLINDER (WERKELIJKE OPPERVLAKTE) ---\n")
ha_finaal_werk <- calc_ha_exact(kommavlinder_biotoop1_finaal_opp)
cat("Werkelijke oppervlakte biotoop Kommavlinder in scenario:", round(ha_finaal_werk, 2), "ha\n")

rm(bwk_buffer_50m, ku_gefilterd_opp, bwk_clean, ku_clean, som_fracties, som_gemedieerd, 
   kommavlinder_biotoop_gecombineerd_opp, r_binair_bwk, r_binair_gecombineerd, biotoop_opp_lijst)
gc()
```
...
if (exists("kommavlinder_biotoop1_finaal_opp") && !all(is.na(terra::values(kommavlinder_biotoop1_finaal_opp, mat = FALSE)))) {
  werkelijk_scenario_rast <- terra::ifel(!is.na(kommavlinder_biotoop1_finaal_opp) & kommavlinder_biotoop1_finaal_opp > 0, 1, NA)
} else {
