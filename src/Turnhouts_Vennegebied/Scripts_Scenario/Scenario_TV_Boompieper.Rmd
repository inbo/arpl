...
```{r clustering}
# ==============================================================================
# FUNCTIE: Clusteren (max 50m via C++ distance) en Filteren op oppervlakte (>= min_opp_ha)
# ==============================================================================
verwerk_leefgebied <- function(r_input, max_afstand_m = 50, min_opp_ha = 5) {
  ...
  # 2. Snelle C++ distance i.p.v. zware focal matrix (Crash-vrij!)
  dist_r <- terra::distance(r_binair, target = 1, maxdist = max_afstand_m + 10)
  r_dilated <- terra::ifel(dist_r <= max_afstand_m, 1, NA)
  rm(dist_r)
  
  # 3. Clusters toekennen over de overbrugde zones
  clusters_dilated <- terra::patches(r_dilated, directions = 8, zeroAsNA = TRUE)
  rm(r_dilated)
  
  # 4. Terugknippen naar de originele voetafdruk
  r_clusters <- terra::mask(clusters_dilated, r_binair)
  
  # 5. Som van werkelijke fracties per cluster
  cluster_opp <- terra::zonal(r_input, r_clusters, fun = "sum", na.rm = TRUE)
  colnames(cluster_opp) <- c("patches", "opp_sum")
  
  # Fractie-som x 100m² -> opp_sum * 0.01 ha
  geldige_ids <- cluster_opp$patches[(cluster_opp$opp_sum * 0.01) >= min_opp_ha]
  
  if (length(geldige_ids) == 0) {
    return(r_input * NA)
  }
  
  masker_geldig <- r_clusters %in% geldige_ids
  r_leefgebied_scherp <- terra::mask(r_input, terra::ifel(masker_geldig == 1, 1, NA))
  
  return(r_leefgebied_scherp)
}

message("-> Clusteren en filteren van BWK-lagen op werkelijke oppervlakte (>= 5 ha)...")
bwk_cluster_opp <- verwerk_leefgebied(bwk_opp, max_afstand_m = 50, min_opp_ha = 5)
```

```{r randzones}
# ==============================================================================
# STAP 1: Randzones bepalen (Heide grenzend aan Naaldbos) - Snelle Distance
# ==============================================================================
message("-> Randzones (edges) bepalen tussen heide en naaldbos...")

if(!is.null(naaldbos_opp) && !is.null(heide_opp)) {
  naaldbos_bin_opp  <- terra::ifel(!is.na(naaldbos_opp) & naaldbos_opp > 0, 1, NA)
  
  # Gebruik distance (<= 15m) i.p.v. focal matrix om de directe bosrand op te pikken
  dist_bos <- terra::distance(naaldbos_bin_opp, target = 1, maxdist = 20)
  naaldbos_edge_opp <- terra::ifel(dist_bos <= 15, 1, NA)
  
  heide_naaldbos_opp <- terra::mask(heide_opp, naaldbos_edge_opp)
  rm(naaldbos_bin_opp, dist_bos, naaldbos_edge_opp)
} else {
  heide_naaldbos_opp <- template_TV * NA
}
```
...
```{r combinatie}
# ==============================================================================
# STAP 3: Combineren (Groot OR Klein)
# ==============================================================================
message("-> Leefgebieden combineren (Groot OF Klein)...")

opp_basis <- terra::cover(bwk_opp, heide_opp)

# Combineer de goedgekeurde grote en kleine leefgebiedfracties
leefgebied_finaal_opp <- terra::cover(
  terra::ifel(is.na(bwk_cluster_opp), NA, opp_basis),
  terra::ifel(is.na(bwk_cluster_klein_opp), NA, opp_basis)
)

# VERBINDING MET EXPORT
final_opp <- leefgebied_finaal_opp

message("--- BOOMPIEPER SCENARIO MODEL AFGEROND ---")
cat("Totaal werkelijk habitat Boompieper binnen scenario (ha):", round(calc_ha_exact(final_opp), 2), "\n")

rm(bwk_cluster_opp, bwk_cluster_klein_opp, opp_basis)
gc()
```
