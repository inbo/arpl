...
if (!is.null(opp_bron) && !all(is.na(terra::values(opp_bron, mat = FALSE)))) {
  ...
  adder_leefgebied_opp <- terra::mask(opp_bron, res_opp$raster) %>% terra::crop(template_TV)
} else {
  adder_leefgebied_opp <- template_TV * NA
}

cat("\n--- AUDIT: SCENARIO LEEFGEBIED ADDER (WERKELIJKE OPPERVLAKTE) ---\n")
ha_finaal_werk <- calc_ha_exact(adder_leefgebied_opp)
cat("Werkelijke oppervlakte leefgebied Adder in scenario:", round(ha_finaal_werk, 2), "ha\n")
...
if (exists("adder_leefgebied_opp") && !all(is.na(terra::values(adder_leefgebied_opp, mat = FALSE)))) {
  werkelijk_scenario_rast <- terra::ifel(!is.na(adder_leefgebied_opp) & adder_leefgebied_opp > 0, 1, NA)
} else {
