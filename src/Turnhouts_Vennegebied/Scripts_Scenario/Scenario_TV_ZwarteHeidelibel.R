...
# ==============================================================================
# BEREIDING WATERBIOTOOP ZWARTEHEIDELIBEL (Minimaal 100 m² - PARALLEL)
# ==============================================================================
...
# ==============================================================================
# BEREIDING RUIME OMGEVING & ZUIVERE PERCENTAGE-CHECK (>40% BOS - PARALLEL)
# ==============================================================================
message("-> Landbiotoop ruime omgeving patches groeperen (50m) en filteren op min. 40 ha...")
...
if (!all(is.na(terra::values(r_patch_ids_max, mat=FALSE)))) {
  ...
  if (length(goedgekeurde_patch_ids_max) > 0) {
    zwarteheidelibel_land_ruim_max <- terra::ifel(r_patch_ids_max %in% goedgekeurde_patch_ids_max, 1, NA)
  }
...
if (!all(is.na(terra::values(r_patch_ids_opp, mat=FALSE)))) {
  ...
  if (length(goedgekeurde_patch_ids_opp) > 0) {
    zwarteheidelibel_land_ruim_opp <- terra::mask(land_ruim_clusters_opp$raster, r_patch_ids_opp %in% goedgekeurde_patch_ids_opp)
  }
...
# ==============================================================================
water_buffer_20m_max <- terra::buffer(r_water1_max_clean, width = 20)
land_buffer_20m_max  <- terra::buffer(zwarteheidelibel_land_nabij_max, width = 20)

waterbiotoop2_max <- terra::mask(r_water1_max_clean, land_buffer_20m_max)
landbiotoop2_max  <- terra::mask(zwarteheidelibel_land_nabij_max, water_buffer_20m_max)

ruim_land_buffer_500m_max <- terra::buffer(zwarteheidelibel_land_ruim_max, width = 500)
...
zwarteheidelibel_leefgebied_max <- terra::cover(waterbiotoop_finaal_max, landbiotoop_finaal_max)
zwarteheidelibel_leefgebied_max <- terra::crop(zwarteheidelibel_leefgebied_max, template_TV)
leefgebied_max    <- zwarteheidelibel_leefgebied_max
finaal_max_binair <- terra::ifel(!is.na(zwarteheidelibel_leefgebied_max) & zwarteheidelibel_leefgebied_max > 0, 1, NA)
...
# ==============================================================================
water_buffer_20m_opp <- terra::buffer(!is.na(r_water1_opp_clean) & r_water1_opp_clean > 0, width = 20)
land_buffer_20m_opp  <- terra::buffer(!is.na(zwarteheidelibel_land_nabij_opp) & zwarteheidelibel_land_nabij_opp > 0, width = 20)

waterbiotoop2_opp <- terra::mask(r_water1_opp_clean, land_buffer_20m_opp)
landbiotoop2_opp  <- terra::mask(zwarteheidelibel_land_nabij_opp, water_buffer_20m_opp)

ruim_land_buffer_500m_opp <- terra::buffer(!is.na(zwarteheidelibel_land_ruim_opp) & zwarteheidelibel_land_ruim_opp > 0, width = 500)
...
zwarteheidelibel_leefgebied_opp <- terra::ifel(som_cl > 0, som_cl, NA)
zwarteheidelibel_leefgebied_opp <- terra::crop(zwarteheidelibel_leefgebied_opp, template_TV)

leefgebied_opp    <- zwarteheidelibel_leefgebied_opp
...
rm(water_buffer_20m_max, land_buffer_20m_max, waterbiotoop2_max, landbiotoop2_max, ruim_land_buffer_500m_max,
   water_buffer_20m_opp, land_buffer_20m_opp, waterbiotoop2_opp, landbiotoop2_opp, ruim_land_buffer_500m_opp,
   w_clean, l_clean, som_raw, som_cl, r_water1_max_clean, r_water1_opp_clean,
   zwarteheidelibel_land_nabij_max, zwarteheidelibel_land_nabij_opp, zwarteheidelibel_land_ruim_max, zwarteheidelibel_land_ruim_opp)
gc()

message("-> Zwarte heidelibel modellering voltooid met water-first afsnijding op 500m!")
