...
masker_bron_max <- bwk_max
masker_bron_max[masker_bron_max == 0] <- NA
opp_bron <- bwk_opp

# --- SPOOR A: MAXIMALE POTENTIE (Binair) ---
res_max <- cluster_filter_compleet(
  masker     = masker_bron_max, 
  opp_laag   = opp_bron, 
...
res_opp <- cluster_filter_compleet(
  masker     = r_binair_opp, 
  opp_laag   = opp_bron, 
  drempel_m2 = drempel_m2, 
  dist_m     = afstand_m, 
...
rm(res_max, res_opp, r_binair_opp, masker_bron_max)
gc()
