...
cluster_filter_compleet <- function(masker, opp_laag, drempel_m2, dist_m, werkelijk = TRUE) {
  ...
  # 3. Bereken de oppervlakte van het GEHELE netwerk op basis van de werkelijke som van fracties
  if(werkelijk) {
  ...
  # 6. Terugknippen naar de fysieke biotoop met behoud van netwerk-IDs
...
# Binaire 'mal' van het studiegebied (1 binnen buffer, 0 buiten)
...
# Enkel het werkelijke oppervlaktespoor doorrekenen
res_opp   <- cluster_filter_compleet(masker_bron, opp_bron, drempel_m2, afstand_m, werkelijk = TRUE)
final_opp <- res_opp$raster

message("--- BLAUWBORST SCENARIO MODEL AFGEROND ---")
cat("Totaal werkelijk habitat Blauwborst binnen scenario (ha):", round(calc_ha_exact(final_opp), 2), "\n")

rm(res_opp, masker_bron, opp_bron)
gc()
```
