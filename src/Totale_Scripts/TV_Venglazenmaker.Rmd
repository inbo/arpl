...
# ==============================================================================
# FINALE RUIMTELIJKE INTERACTIES EN SAMENSMELTING (SPOOR MAX & SPOOR OPP)
# ==============================================================================
message("-> Ruimtelijke interacties en eindsynthese uitvoeren op basis van GEHELE CLUSTERS...")

# Opschonen en isoleren van de basis waterbiotopen
r_water1_max_clean <- waterbiotoop_clusters_max$raster
r_water1_opp_clean <- waterbiotoop_clusters_opp$raster

# ------------------------------------------------------------------------------
# SPOOR A: MAX (Potentieel Leefgebied - Binair 1/NA)
# ------------------------------------------------------------------------------
message("    - Spoor MAX verwerken...")

# Step 1: Binaire buffer van 20m rond het water
water_buf20_max_bin <- terra::buffer(!is.na(r_water1_max_clean) & r_water1_max_clean > 0, width = 20)

# Step 2: Bepaal welke goedgekeurde landclusters binnen 20m van het water liggen
...
# Step 3: Behou de VOLLEDIGE landclusters die gekoppeld zijn (ZONDER 500m knip)
if (length(actieve_land_ids_max) > 0) {
  m_land_gekoppeld_max <- terra::match(r_nabij_patch_ids_max, actieve_land_ids_max)
  venglazenmaker_landbiotoop_max <- terra::ifel(!is.na(m_land_gekoppeld_max), 1, NA)
} else {
  venglazenmaker_landbiotoop_max <- template_TV * NA
}

# Step 4: Water koppelen aan land (water moet binnen 20m van goedgekeurd land liggen)
...
# Step 5: Samenvoegen tot maximaal potentieel leefgebied
venglazenmaker_leefgebied_max <- terra::cover(venglazenmaker_waterbiotoop_max, venglazenmaker_landbiotoop_max)
venglazenmaker_leefgebied_max <- terra::ifel(!is.na(venglazenmaker_leefgebied_max), 1, NA)


# ------------------------------------------------------------------------------
# SPOOR B: OPP (Actueel Leefgebied - Werkelijke Oppervlakte per cel)
# ------------------------------------------------------------------------------
message("    - Spoor OPP verwerken...")

# Step 1: Binaire buffers van water voor 20m interactie
...
# Step 3: Behou de VOLLEDIGE landclusters met werkelijke oppervlaktes (ZONDER 500m knip)
if (length(actieve_land_ids_opp) > 0) {
  ...
  venglazenmaker_landbiotoop_opp <- terra::mask(land_nabij_filter_opp, winnende_masker_opp)
} else {
  venglazenmaker_landbiotoop_opp <- template_TV * NA
}

# Step 4: Water koppelen aan land
...
# Step 5: Werkelijke oppervlaktes optellen (met een max van 1.0 ha / cel)
opp_stack <- c(venglazenmaker_waterbiotoop_opp, venglazenmaker_landbiotoop_opp)
...
# ------------------------------------------------------------------------------
# FINALE CROP OP HET STUDIEGEBIED (Turnhouts Vennegebied)
# ------------------------------------------------------------------------------
...
message("-> Finale samensmelting succesvol afgerond!")
```
...
# ==============================================================================
# STAP 8: INTERACTIEVE KAART (LEAFLET) - WERKELIJK LEEFGEBIED + HA PER METACLUSTER
# ==============================================================================
if (exists("r_net_ids_tv") && !all(is.na(terra::values(r_net_ids_tv, mat=FALSE)))) {
  
  message("-> Leefgebied polygonen genereren met exacte oppervlakteberekening per metacluster...")
  
  # 1. Bepaal de oppervlakte (ha) per uniek Metacluster ID op het werkelijke habitat (finaal_opp_binair)
  ha_per_cluster <- terra::zonal(
    finaal_opp_binair * terra::cellSize(finaal_opp_binair, unit = "ha"), 
    r_net_ids_tv, 
    fun = "sum", 
    na.rm = TRUE
  )
  colnames(ha_per_cluster) <- c("Netwerk_ID", "Opp_ha")
  ha_per_cluster$Opp_ha <- round(ha_per_cluster$Opp_ha, 2)
  
  # 2. Polygonen maken van het WERKLIJKE LEEFGEBIED (zowel land als water)
  # Snijd r_net_ids_tv af tot waar werkelijk habitat aanwezig is
  r_werkelijk_net_ids <- terra::mask(r_net_ids_tv, terra::ifel(!is.na(finaal_opp_binair) & finaal_opp_binair > 0, 1, NA))
  
  cl_poly <- terra::as.polygons(r_werkelijk_net_ids, aggregate = TRUE) %>% 
    st_as_sf() %>% 
    st_transform(4326)
  
  colnames(cl_poly)[1] <- "Netwerk_ID"
  
  # 3. Koppel de oppervlakte en status aan de polygonen
  cl_poly_final <- cl_poly %>%
    left_join(ha_per_cluster, by = "Netwerk_ID") %>%
    mutate(
      Is_ARPL = Netwerk_ID %in% ids_actueel_relevant,
      Is_Verspr = Netwerk_ID %in% ids_verspreiding_clusters,
      Status_Label = case_when(
        Is_Verspr ~ "Actuele Verspreiding (Exacte match 0m)",
        Is_ARPL   ~ "Actueel Relevant (ARPL - Binnen dispersie)",
        TRUE      ~ "Werkelijk Habitat (Geen recente waarnemingen)"
      ),
      Popup = paste0(
        "<div style='font-family: Arial; min-width: 220px;'>",
        "<strong>Metacluster ID:</strong> ", Netwerk_ID, "<br>",
        "<strong>Soort:</strong> Venglazenmaker<br><hr>",
        "<strong>Oppervlakte:</strong> <b style='color:#2b8cbe;'>", Opp_ha, " ha</b><br>",
        "<strong>Status:</strong> ", Status_Label, "<br>",
        "</div>"
      )
    )
    
  # 4. Geschikte waterbiotopen voorbereiden als extra overlay
...
# Kleurenpalette per status
pal_status <- colorFactor(
  palette = c("#2b8cbe", "#e41a1c", "#4daf4a"), 
  levels = c("Actuele Verspreiding (Exacte match 0m)", "Actueel Relevant (ARPL - Binnen dispersie)", "Werkelijk Habitat (Geen recentie waarnemingen)")
)
...
# 1. Voeg het WERKLIJKE LEEFGEBIED (Metaclusters) toe
if (exists("cl_poly_final") && nrow(cl_poly_final) > 0) {
  map <- map %>%
    addPolygons(
      data = cl_poly_final,
      fillColor = ~pal_status(Status_Label),
      fillOpacity = 0.6, 
      color = "#1c9099", 
      weight = 1.5, 
      popup = ~Popup,
      group = "Werkelijk Leefgebied (Metaclusters)"
    )
}

# 2. Voeg de geschikte waterbiotopen toe als accent
...
# 3. Voeg dispersiebereik (ARPL buffer_m) en exacte waarnemingspunten toe
if(has_observations && nrow(df_points_sf) > 0) {
  map <- map %>%
    addPolygons(
      data = waarneming_smear_web,  
      color = "#e41a1c", weight = 1.5, fill = FALSE,
      dashArray = "6, 4", group = "Dispersiebereik ARPL (buffer_m)"
    ) %>%
    addCircles(
      data = points_web, 
      color = "black", weight = 1, fillColor = "orange", fillOpacity = 1,
      radius = 12, group = "Waarnemingen (0m)"
    )
}
...
map %>%
  ...
  addLayersControl(
    ...
    overlayGroups = c("Werkelijk Leefgebied (Metaclusters)", "Geschikte Waterbiotopen", 
                      "Dispersiebereik ARPL (buffer_m)", "Waarnemingen (0m)", "Grens Studiegebied"),
