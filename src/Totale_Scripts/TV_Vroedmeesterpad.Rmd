...
if (exists("r_net_max_tv") && !all(is.na(terra::values(r_net_max_tv, mat=FALSE)))) {
  ...
  stats_global_opp <- terra::zonal(vroedmeesterpad_leefgebied_max * 0.01, r_opp_net_ids_max, fun = "sum", na.rm = TRUE)
...
if (exists("r_vroedmeesterpad_waterbiotoop1") && !all(is.na(terra::values(r_vroedmeesterpad_waterbiotoop1, mat=FALSE)))) {
  r_web_vpt <- terra::project(r_vroedmeesterpad_waterbiotoop1, "EPSG:4326", method = "near")
} else {
  r_web_vpt <- NULL
}
...
```{r export}
# ==============================================================================
# SCHONE BINAIR EXPORT ZONDER VALSE NULLEN (VROEDMEESTERPAD - TURNHOUTS VENNEGEBIED)
# ==============================================================================
...
# 1. Maximale potentie
if (exists("vroedmeesterpad_leefgebied_max") && !all(is.na(terra::values(vroedmeesterpad_leefgebied_max, mat = FALSE)))) {
  potentie_export_rast <- terra::ifel(!is.na(vroedmeesterpad_leefgebied_max) & vroedmeesterpad_leefgebied_max > 0, 1, NA)
...
if (exists("vroedmeesterpad_leefgebied_opp") && !all(is.na(terra::values(vroedmeesterpad_leefgebied_opp, mat = FALSE)))) {
  werkelijk_export_rast <- terra::ifel(!is.na(vroedmeesterpad_leefgebied_opp) & vroedmeesterpad_leefgebied_opp > 0, 1, NA)
...
# 3. ARPL
if (exists("vroedmeesterpad_arpl_opp") && !all(is.na(terra::values(vroedmeesterpad_arpl_opp, mat = FALSE)))) {
  arpl_export_rast <- terra::ifel(!is.na(vroedmeesterpad_arpl_opp) & vroedmeesterpad_arpl_opp > 0, 1, NA)
...
if (exists("vroedmeesterpad_verspreiding_bin") && !all(is.na(terra::values(vroedmeesterpad_verspreiding_bin, mat = FALSE)))) {
  verspreiding_export_rast <- terra::ifel(!is.na(vroedmeesterpad_verspreiding_bin) & vroedmeesterpad_verspreiding_bin > 0, 1, NA)
...
message("🏁 SCRIPT EN EXPORT VOLLEDIG EN SUCCESVOL AFGEROND VOOR VROEDMEESTERPAD!")
```
