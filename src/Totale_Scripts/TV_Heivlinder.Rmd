...
```{r export heivlinder}
# ==============================================================================
# SCHONE BINAIR EXPORT ZONDER VALSE NULLEN (HEIVLINDER - TURNHOUTS VENNEGEBIED)
# ==============================================================================
message("-> Start geformatteerde binaire export voor heivlinder...")
...
# 1. Maximale potentie
if (exists("heivlinder_leefgebied_metzand_max") && !all(is.na(terra::values(heivlinder_leefgebied_metzand_max, mat = FALSE)))) {
  potentie_export_rast <- terra::ifel(!is.na(heivlinder_leefgebied_metzand_max) & heivlinder_leefgebied_metzand_max > 0, 1, NA)
} else {
...
if (exists("heivlinder_arpl_opp") && !all(is.na(terra::values(heivlinder_arpl_opp, mat = FALSE)))) {
  arpl_export_rast <- terra::ifel(!is.na(heivlinder_arpl_opp) & heivlinder_arpl_opp > 0, 1, NA)
} else {
  arpl_export_rast <- terra::rast(template_TV, vals = NA)
}

# 4. Actuele verspreiding
if (exists("heivlinder_verspreiding_bin") && !all(is.na(terra::values(heivlinder_verspreiding_bin, mat = FALSE)))) {
  verspreiding_export_rast <- terra::ifel(!is.na(heivlinder_verspreiding_bin) & heivlinder_verspreiding_bin > 0, 1, NA)
} else {
...
message("-> Alle GeoTIFF exports voor de Heivlinder zijn succesvol weggeschreven!")
```