...
scenario_bestanden <- c(here("data/input/Scenario_gpkg/streefbeelden_allocatie_tvg_geenmutatie_vrij_20260917_r1_voorsoorten.gpkg"))
...
tabel_bwk_lokaal_global <- readRDS(here("data/input/Raster_Vlaanderen/BWK_Tabel_Turnhouts_Vennegebied_Plus_50km.rds"))
...
for(scen_path in scenario_bestanden) {
  ...
  export_pad  <- here("data/input/Scenario_rds", export_naam)
  saveRDS(tabel_finaal_scenario, export_pad)
  
  scen_duur <- difftime(Sys.time(), scen_tijd_start, units="mins")
  message(paste("✓ Scenario", basis_naam, "is succesvol verrasterd en opgeslagen als:", export_pad, "in", round(scen_duur, 2), "minuten."))
