...
# ==============================================================================
# 0. INITIALISEER ALLE TELLERS EN VARIABELEN HARDE OP 0 VOORAF
# ==============================================================================
...
# ==============================================================================
# 1. WAARNEMINGEN INLADEN (ROBUUST MET DIRECTE X/Y LAMBERT 72)
# ==============================================================================
...
if (file.exists(waarnemingen_path)) {
  ...
  waarnemingen_clean <- waarnemingen_raw %>% filter(!is.na(x) & !is.na(y))
  
  if (nrow(waarnemingen_clean) > 0) {
    df_points_sf          <- st_as_sf(waarnemingen_clean, coords = c("x", "y"), crs = 31370)
    waarneming_buffer_ruw <- st_buffer(df_points_sf, dist = buffer_m)
    waarneming_union      <- st_union(waarneming_buffer_ruw)
    
    waarneming_smear_sf   <- st_sf(id = 1, geometry = st_cast(waarneming_union, "MULTIPOLYGON"))
...
if (!has_observations) {
  df_points_sf           <- st_sf(geometry = st_sfc(crs = 31370)) 
  waarneming_smear_sf    <- st_sf(geometry = st_sfc(crs = 31370))
  waarneming_blobs_vect  <- terra::vect(crs = "EPSG:31370")
  waarneming_punten_vect <- terra::vect(crs = "EPSG:31370")
}
...
# ==============================================================================
# SCHONE BINAIR EXPORT ZONDER VALSE NULLEN (GLADDE SLANG - TURNHOUTS VENNEGEBIED)
# ==============================================================================
...
# 3. ARPL
if (exists("gladdeslang_arpl_opp") && !all(is.na(terra::values(gladdeslang_arpl_opp, mat = FALSE)))) {
...
if (exists("gladdeslang_verspreiding_bin") && !all(is.na(terra::values(gladdeslang_verspreiding_bin, mat = FALSE)))) {
  verspreiding_export_rast <- terra::ifel(!is.na(gladdeslang_verspreiding_bin) & gladdeslang_verspreiding_bin > 0, 1, NA)
} else {
...
message("-> Alle GeoTIFF exports voor de Gladde slang in Kalmthoutse Heide zijn succesvol weggeschreven!")
```
