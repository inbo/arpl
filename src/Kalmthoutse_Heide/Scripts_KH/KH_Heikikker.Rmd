...
if (!all(is.na(terra::values(heikikker_leefgebied_opp, mat=FALSE)))) {
  ...
  if (has_observations && nrow(df_points_sf) > 0) {
    ...
    if(length(ids_arpl) > 0) {
      ...
      heikikker_arpl_opp <- terra::mask(r_opp_tv, terra::crop(terra::ifel(m_arpl == 1, 1, NA), template_KH))
...
```{r export}
# ==============================================================================
# SCHONE BINAIR EXPORT ZONDER VALSE NULLEN (HEIKIKKER - TURNHOUTS VENNEGEBIED)
# ==============================================================================
...
# 1. Maximale potentie
if (exists("heikikker_leefgebied_max") && !all(is.na(terra::values(heikikker_leefgebied_max, mat = FALSE)))) {
  potentie_export_rast <- terra::ifel(!is.na(heikikker_leefgebied_max) & heikikker_leefgebied_max > 0, 1, NA)
...
if (exists("heikikker_leefgebied_opp") && !all(is.na(terra::values(heikikker_leefgebied_opp, mat = FALSE)))) {
...
if (exists("heikikker_arpl_opp") && !all(is.na(terra::values(heikikker_arpl_opp, mat = FALSE)))) {
  arpl_export_rast <- terra::ifel(!is.na(heikikker_arpl_opp) & heikikker_arpl_opp > 0, 1, NA)
...
if (exists("heikikker_verspreiding_bin") && !all(is.na(terra::values(heikikker_verspreiding_bin, mat = FALSE)))) {
  verspreiding_export_rast <- terra::ifel(!is.na(heikikker_verspreiding_bin) & heikikker_verspreiding_bin > 0, 1, NA)
