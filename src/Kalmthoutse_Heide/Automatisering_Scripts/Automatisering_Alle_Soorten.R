...
MAP_WAARNEMINGEN   <- here("data/input/Waarnemingen_Soorten/Kalmthoutse_Heide")
...
# ------------------------------------------------------------------------------
alle_tv_scripts <- list.files(path = MAP_SCRIPTS_KH, pattern = "\\.Rmd$", full.names = TRUE)
soorten_script_tv_pad <- file.path(MAP_SCRIPTS_KH, "KH_Leefgebieden_Simpel.Rmd")
unieke_tv_scripts <- setdiff(alle_tv_scripts, soorten_script_tv_pad)

message("=> Gedetecteerde unieke KH-scripts om te knitten (Aantal: ", length(unieke_tv_scripts), "):")
...
message("\n=> Aantal geselecteerde simpele soorten voor KH-run (met waarnemingenfile): ", length(soorten_lijst))
