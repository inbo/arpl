# ==============================================================================
# MASTER AUTOMATISERINGSSCRIPT VOOR REGULIERE KH-SCRIPTS (KNITTEN)
...
# ------------------------------------------------------------------------------
MAP_SCRIPTS_KH   <- here("src/Kalmthoutse_Heide/Scripts_KH")
OUTPUT_DIR       <- here("data/output/Kalmthoutse_Heide/HTML_Rapporten_Soorten")
...
message("==================================================")
message(" START MASTER RUN: NORMALE KH SCRIPTS KNITTEN     ")
message("==================================================")
...
# ------------------------------------------------------------------------------
# 1. DETECTEER EN KNIT DE UNIEKE KH-SCRIPTS
# ------------------------------------------------------------------------------
alle_kh_scripts <- list.files(path = MAP_SCRIPTS_KH, pattern = "\\.Rmd$", full.names = TRUE)
soorten_script_kh_pad <- file.path(MAP_SCRIPTS_KH, "KH_Leefgebieden_Simpel.Rmd")
unieke_kh_scripts <- setdiff(alle_kh_scripts, soorten_script_kh_pad)

message("=> Gedetecteerde unieke KH-scripts om te knitten (Aantal: ", length(unieke_kh_scripts), "):")
print(basename(unieke_kh_scripts))
message("--------------------------------------------------")

for(script in unieke_kh_scripts) {
  ...
  if (!moet_knitten(output_html, OUTPUT_DIR)) {
    ...
    algemeen_logboek[[bestandsnaam]] <- data.frame(
      Item = bestandsnaam, Type = "Uniek KH Script", Status = "GEKOZEN_OVERSLAAN", Fout = "Reeds vandaag geknit", stringsAsFactors = FALSE
    )
  ...
  tryCatch({
    ...
    algemeen_logboek[[bestandsnaam]] <- data.frame(
      Item = bestandsnaam, Type = "Uniek KH Script", Status = "SUCCES", Fout = "Geen", stringsAsFactors = FALSE
    )
  }, error = function(e) {
    message("❌ FOUT bij: ", bestandsnaam)
    algemeen_logboek[[bestandsnaam]] <- data.frame(
      Item = bestandsnaam, Type = "Uniek KH Script", Status = "CRASH", Fout = e$message, stringsAsFactors = FALSE
    )
  })
}
...
soorten_lijst <- excel_data %>% 
  ...
  filter(Kalmthoutse_Heide == 1) %>% 
  pull(Soort) %>% 
  unique() %>% 
  na.omit()

message("\n=> Aantal geselecteerde simpele soorten voor KH-run: ", length(soorten_lijst))


# ------------------------------------------------------------------------------
# 3. RUN HET 'SIMPEL_KH' SCRIPT PER SOORT (Knitten met parameter)
# ------------------------------------------------------------------------------
draai_leefgebied_model <- function(huidige_soort, script_pad) {
  output_file_name <- paste0("KH_", huidige_soort, ".html")
...
if (length(soorten_lijst) > 0) {
  soorten_logboek <- purrr::map_dfr(
    soorten_lijst, 
    ~draai_leefgebied_model(huidige_soort = .x, script_pad = soorten_script_kh_pad)
  )
...
# ------------------------------------------------------------------------------
log_file_path <- file.path(OUTPUT_DIR, paste0("Logboek_KH_NormaalRun_", format(Sys.time(), "%Y%m%d_%H%M"), ".csv"))
...
message("\n==================================================")
message(" MASTER RUN KH NORMAAL COMPLEET!                  ")
