...
if (!is.null(r_witsnuit_waterbiotoop1) && !all(is.na(suppressWarnings(terra::minmax(r_witsnuit_waterbiotoop1))))) {
  ...
  if (!all(is.na(suppressWarnings(terra::minmax(cl_water_cand_max))))) {
    ...
    } else { r_water_cand_max <- template_KH * NA }
  } else { r_water_cand_max <- template_KH * NA }
...
if (!is.null(r_witsnuit_waterbiotoop1_opp) && !all(is.na(suppressWarnings(terra::minmax(r_witsnuit_waterbiotoop1_opp))))) {
  ...
  if (!all(is.na(suppressWarnings(terra::minmax(cl_water_cand_opp))))) {
    ...
    } else { r_water_cand_opp <- template_KH * NA }
  } else { r_water_cand_opp <- template_KH * NA }
  ...
  if (!all(is.na(suppressWarnings(terra::minmax(gevlektewitsnuitlibel_voortplanting_max))))) {
    ...
    gevlektewitsnuitlibel_voortplanting_opp <- template_KH * NA
    gevlektewitsnuitlibel_water_finaal_opp <- template_KH * NA
  }
} else {
  gevlektewitsnuitlibel_voortplanting_opp <- template_KH * NA
  gevlektewitsnuitlibel_water_finaal_opp <- template_KH * NA
...
} else { land_ruim_ok_max <- template_KH * NA }
...
} else { land_ruim_ok_opp <- template_KH * NA }
...
if (exists("gevlektewitsnuitlibel_voortplanting_max") && !all(is.na(suppressWarnings(terra::minmax(gevlektewitsnuitlibel_voortplanting_max))))) {
  ...
  if (exists("land_nabij_max") && !all(is.na(suppressWarnings(terra::minmax(land_nabij_max))))) {
    ...
    if (!all(is.na(suppressWarnings(terra::minmax(land_5ha_max))))) {
      r_struweel_max_clean <- fix_bos_raster(struweel1_max, template_KH)
      ...
      land_ok_max <- if(length(valide_ids_max) > 0) terra::mask(land_5ha_max, terra::ifel(cl_id_5ha_max %in% valide_ids_max, 1, NA)) else template_KH * NA
    } else { land_ok_max <- template_KH * NA }
  ...
  } else { gevlektewitsnuitlibel_land_finaal_max <- template_KH * NA }
} else { gevlektewitsnuitlibel_land_finaal_max <- template_KH * NA }

water_finaal_max  <- if(exists("gevlektewitsnuitlibel_water_finaal_max")) gevlektewitsnuitlibel_water_finaal_max else template_KH * NA
...
if (exists("gevlektewitsnuitlibel_voortplanting_opp") && !all(is.na(suppressWarnings(terra::minmax(gevlektewitsnuitlibel_voortplanting_opp))))) {
  ...
  if (exists("land_nabij_opp") && !all(is.na(suppressWarnings(terra::minmax(land_nabij_opp))))) {
    ...
    if (!all(is.na(suppressWarnings(terra::minmax(land_5ha_opp))))) {
      r_struweel_opp_clean <- fix_bos_raster(struweel1_opp, template_KH)
      ...
      land_ok_opp <- if(length(valide_ids_opp) > 0) terra::mask(land_5ha_opp, terra::ifel(cl_id_5ha_opp %in% valide_ids_opp, 1, NA)) else template_KH * NA
    } else { land_ok_opp <- template_KH * NA }
  ...
  } else { gevlektewitsnuitlibel_land_finaal_opp <- template_KH * NA }
} else { gevlektewitsnuitlibel_land_finaal_opp <- template_KH * NA }

water_finaal_opp <- if(exists("gevlektewitsnuitlibel_voortplanting_opp")) gevlektewitsnuitlibel_voortplanting_opp else template_KH * NA
