...
groenkaart_10m_synchronized <- project(hooggroen_raw, id_raster_KH, method = "max")
...
if (!all(is.na(suppressWarnings(terra::minmax(voortplanting_base_max))))) {
  ...
  foerageer_finaal_max     <- template_KH * NA
  voortplanting_finaal_max <- template_KH * NA
}
...
if (!all(is.na(suppressWarnings(terra::minmax(voortplanting_base_opp))))) {
  ...
  foerageer_finaal_opp     <- template_KH * NA
  voortplanting_finaal_opp <- template_KH * NA
}
