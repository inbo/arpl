...
# Expert-parameters Poelkikker
opp_water_ha   <- 0.025  # MinOpp waterbiotoop (0,025 ha = 250 m2)
opp_land_ha    <- 10.0   # MinOpp landbiotoop per eiland (10 ha)
buffer_m       <- 1200   # Dispersiecapaciteit (1200 m)
...
message("-> Watervlakken shapefile inlezen en verwerken voor poelkikker...")
...
if (!is.null(kleine_clusters_v) && nrow(kleine_clusters_v) > 0) {
  ...
  if (!is.null(kleine_watervlakken_finaal) && nrow(kleine_watervlakken_finaal) > 0) {
    r_pk_kleinwater <- terra::rasterize(kleine_watervlakken_finaal, template_KH, field = 1, background = NA)
    
    r_poelkikker_waterbiotoop1     <- terra::cover(waterbiotoop_bwk_max, r_pk_kleinwater)
    r_poelkikker_waterbiotoop1_opp <- terra::cover(waterbiotoop_bwk_opp, r_pk_kleinwater)
  } else {
    r_poelkikker_waterbiotoop1     <- waterbiotoop_bwk_max
    r_poelkikker_waterbiotoop1_opp <- waterbiotoop_bwk_opp
  }
} else {
  r_poelkikker_waterbiotoop1     <- waterbiotoop_bwk_max
  r_poelkikker_waterbiotoop1_opp <- waterbiotoop_bwk_opp
...
suppressWarnings(
  rm(watervlakken_v, watervlakken_buffered, watervlakken_fuzzy_clusters, kleine_clusters_v, kleine_watervlakken_finaal, r_pk_kleinwater)
)
gc()
...
if (!all(is.na(terra::values(landbiotoop_bwk_max, mat = FALSE)))) {
  ...
  if (length(ids_land_ok_max) > 0) {
    poelkikker_land_gefilterd_max <- terra::mask(landbiotoop_bwk_max, land_patches_max %in% ids_land_ok_max)
  } else {
    poelkikker_land_gefilterd_max <- template_KH * NA
  }
} else {
  poelkikker_land_gefilterd_max <- template_KH * NA
}
...
if (!all(is.na(terra::values(landbiotoop_bwk_opp, mat = FALSE)))) {
  ...
  if (length(ids_land_ok_opp) > 0) {
    poelkikker_land_gefilterd_opp <- terra::mask(landbiotoop_bwk_opp, land_patches_opp %in% ids_land_ok_opp)
  } else {
    poelkikker_land_gefilterd_opp <- template_KH * NA
  }
} else {
  poelkikker_land_gefilterd_opp <- template_KH * NA
}

message("-> Ruimtelijke koppeling land- en waterbiotoop (AfstandBiotopen = 0 m + 10m oeverkoppling) voor MAX en OPP parallel...")

# ==============================================================================
# SPOOR MAX (Volledige waterplassen behouden die aan land grenzen)
# ==============================================================================
if (!all(is.na(terra::values(poelkikker_land_gefilterd_max, mat = FALSE))) && 
    !all(is.na(terra::values(r_poelkikker_waterbiotoop1, mat = FALSE)))) {
  
  # Buffer het land 10m om aangrenzende waterpixels volledig mee te grijpen
  r_land_buf_max <- terra::buffer(poelkikker_land_gefilterd_max, width = 10)
  
  # Selecteer de volledige waterplassen die raken aan het gekwalificeerde land
  pk_water_gekoppeld_max <- terra::mask(r_poelkikker_waterbiotoop1, r_land_buf_max)
  
  # Voeg het volledige landbiotoop en het volledige waterbiotoop samen
  poelkikker_leefgebied1_max <- terra::cover(poelkikker_land_gefilterd_max, pk_water_gekoppeld_max)
  poelkikker_leefgebied1_max <- terra::ifel(!is.na(poelkikker_leefgebied1_max), 1, NA)
} else {
  poelkikker_leefgebied1_max <- template_KH * NA
}

# ==============================================================================
# SPOOR OPP (Volledig autonoom met behoud van volledige wateroppervlaktes)
# ==============================================================================
if (!all(is.na(terra::values(poelkikker_land_gefilterd_opp, mat = FALSE))) && 
    !all(is.na(terra::values(r_poelkikker_waterbiotoop1_opp, mat = FALSE)))) {
  
  # Binaire landkaart maken en 10m buffer toepassen
  r_binair_land_opp <- terra::ifel(!is.na(poelkikker_land_gefilterd_opp) & poelkikker_land_gefilterd_opp > 0, 1, NA)
  r_land_buf_opp    <- terra::buffer(r_binair_land_opp, width = 10)
  
  # Filter het water op basis van de landbuffer
  pk_water_gekoppeld_opp <- terra::mask(r_poelkikker_waterbiotoop1_opp, r_land_buf_opp)
  
  # Samenvoegen op basis van werkelijke bedekkingsgraden
  water_opp_clean <- terra::ifel(is.na(pk_water_gekoppeld_opp), 0, pk_water_gekoppeld_opp)
  land_opp_clean  <- terra::ifel(is.na(poelkikker_land_gefilterd_opp), 0, poelkikker_land_gefilterd_opp)

  som_opp <- terra::clamp(water_opp_clean + land_opp_clean, upper = 1.0)
  poelkikker_leefgebied1_opp <- terra::ifel(som_opp > 0, som_opp, NA)
} else {
  poelkikker_leefgebied1_opp <- template_KH * NA
}

message("-> Netwerkvorming binnen 1200m en controle op >= 160 ha EN >= 5 plassen voor MAX en OPP parallel...")
...
res_netwerk_max <- cluster_filter_compleet(
  masker     = poelkikker_leefgebied1_max,
  opp_laag   = poelkikker_leefgebied1_max,
...
if (!all(is.na(terra::values(cl_netwerk_max, mat = FALSE)))) {
  ...
  if (length(na.omit(valid_ids_max)) > 0) {
    poelkikker_leefgebied_max <- terra::mask(r_netwerk_max, cl_netwerk_max %in% valid_ids_max)
    cl_max                    <- terra::mask(cl_netwerk_max, cl_netwerk_max %in% valid_ids_max)
  } else {
    poelkikker_leefgebied_max <- template_KH * NA
...
# ==============================================================================
r_binair_leef1_opp <- terra::ifel(!is.na(poelkikker_leefgebied1_opp) & poelkikker_leefgebied1_opp > 0, 1, NA)

res_netwerk_opp <- cluster_filter_compleet(
  masker     = r_binair_leef1_opp,
  opp_laag   = poelkikker_leefgebied1_opp,
  drempel_m2 = drempel_netwerk_m2,
  dist_m     = buffer_m,
  werkelijk  = TRUE
)
...
if (!all(is.na(terra::values(cl_netwerk_opp, mat = FALSE)))) {
  ...
  if (length(na.omit(valid_ids_opp)) > 0) {
    poelkikker_leefgebied_opp <- terra::mask(poelkikker_leefgebied1_opp, cl_netwerk_opp %in% valid_ids_opp)
    cl_opp                    <- terra::mask(cl_netwerk_opp, cl_netwerk_opp %in% valid_ids_opp)
  } else {
    poelkikker_leefgebied_opp <- template_KH * NA
...
final_max <- poelkikker_leefgebied_max
final_opp <- poelkikker_leefgebied_opp
