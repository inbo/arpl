...
extractie_ids <- terra::extract(id_raster_KH_globale_values, area_buffer_fix, cells = TRUE)

vertaal_df <- as.data.table(extractie_ids)
setnames(vertaal_df, c("cell", names(id_raster_KH_globale_values)), c("lokale_id", "globale_id"))
...
rm(globale_id_raster, id_raster_KH_globale_values)
gc()
...
r_drain_local <- r_drain_raw %>% terra::crop(area_buffer_fix) %>% terra::resample(template_KH, method = "near")
...
crs(r_grb_water_local) <- crs(template_KH)
...
verwerk_habitat_met_bosrand <- function(r_habitat_max, r_habitat_opp, r_bos_max, r_bos_opp, dist_m = 20, drempel_m2, afstand_m = 0) {
  ...
  if (!terra::hasValues(r_habitat_max) || terra::global(r_habitat_max, "notNA")$notNA == 0) {
    return(list(max = template_KH * NA, opp = template_KH * NA, clusters = template_KH * NA))
  }
  ...
  if (length(voldoet_bos_ids_max) > 0) {
    ...
    complex_raw_max <- template_KH * NA
    cl_finaal_max   <- template_KH * NA
  }
  ...
  if (terra::hasValues(r_habitat_bin_opp) && terra::global(r_habitat_bin_opp, "notNA")$notNA > 0) {
    ...
    if (length(voldoet_bos_ids_opp) > 0) {
      ...
      complex_raw_opp <- template_KH * NA
      cl_finaal_opp   <- template_KH * NA
    }
  } else {
    complex_raw_opp <- template_KH * NA
    cl_finaal_opp   <- template_KH * NA
