...
# ==============================================================================
# REGENWULP: HERZIENE BALANSERING TUSSEN OPEN CULTUURLANDSCHAP EN HEIDE
# ==============================================================================
message("-> 1. Open Cultuurlandschap (Graslanden + Akkers) bundelen en clusteren...")

# Combineer grasland en akkers
open_land_max <- terra::cover(foerageer_max, akkers_max)
open_land_opp <- terra::cover(foerageer_opp, akkers_opp)

drempel_m2 <- oppervlakte_ha * 10000

# Spoor A: Open Land Max
cl_open_land_max <- cluster_filter_compleet(
  masker       = open_land_max,
  opp_laag     = open_land_opp,
  drempel_m2   = drempel_m2, 
  dist_m       = afstand_m, # 50m
  werkelijk    = FALSE
)
r_open_land_finaal_max <- cl_open_land_max$raster

# Spoor B: Open Land Opp
r_binair_open_opp <- terra::ifel(!is.na(open_land_opp) & open_land_opp > 0, 1, NA)
cl_open_land_opp <- cluster_filter_compleet(
...
r_open_land_finaal_opp <- cl_open_land_opp$raster

message("-> 2. Heidegebieden clusteren...")
# Heide Spoor A & B
if (exists("heide_max") && !all(is.na(terra::values(heide_max, mat=FALSE)))) {
  cl_heide_max <- cluster_filter_compleet(
    masker       = heide_max,
    opp_laag     = heide_opp,
    drempel_m2   = drempel_m2, 
    dist_m       = afstand_m,
  ...
  r_heide_finaal_max <- cl_heide_max$raster
  
  r_binair_heide_opp <- terra::ifel(!is.na(heide_opp) & heide_opp > 0, 1, NA)
  cl_heide_opp <- cluster_filter_compleet(
    masker       = r_binair_heide_opp,
    opp_laag     = heide_opp,
    drempel_m2   = drempel_m2, 
    dist_m       = afstand_m,
    werkelijk    = TRUE
  )
  r_heide_finaal_opp <- cl_heide_opp$raster
} else {
  r_heide_finaal_max <- template_KH * NA
  r_heide_finaal_opp <- template_KH * NA
}

message("-> 3. Samenvoegen van Open Cultuurlandschap en Heide...")
regenwulp_leefgebied_max <- terra::cover(r_open_land_finaal_max, r_heide_finaal_max) %>% terra::crop(template_KH)
regenwulp_leefgebied_opp <- terra::cover(r_open_land_finaal_opp, r_heide_finaal_opp) %>% terra::crop(template_KH)

message("--- REGENWULP MODEL HERZIEN EN SUCCESVOL AFGEROND ---")
...
rm(cl_open_land_max, cl_open_land_opp, r_open_land_finaal_max, r_open_land_finaal_opp, open_land_max, open_land_opp, r_binair_open_opp, r_watervogelgebied_max)
gc()
