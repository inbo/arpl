...
template_KH <- terra::rasterize(area_shape_proj, id_raster_KH, field = 1, background = NA)
template_KH <- terra::ifel(!is.na(template_KH), 1, NA)
...
wulp_leefgebied_max <- template_KH * NA
wulp_leefgebied_opp <- template_KH * NA
...
if (exists("foerageer_bwk_max") && !is.null(foerageer_bwk_max) && !all(is.na(suppressWarnings(terra::minmax(foerageer_bwk_max))))) {
  ...
  if (!is.null(r_wulp_30ha_max) && !all(is.na(suppressWarnings(terra::minmax(r_wulp_30ha_max))))) {
    wulp_leefgebied_max <- terra::mask(r_wulp_30ha_max, r_watervogelgebied_max) %>% terra::crop(template_KH)
  }
}
...
if (exists("foerageer_bwk_opp") && !is.null(foerageer_bwk_opp) && !all(is.na(suppressWarnings(terra::minmax(foerageer_bwk_opp))))) {
  ...
  if (!is.null(r_wulp_30ha_opp) && !all(is.na(suppressWarnings(terra::minmax(r_wulp_30ha_opp))))) {
    wulp_leefgebied_opp <- terra::mask(r_wulp_30ha_opp, r_watervogelgebied_max) %>% terra::crop(template_KH)
  }
}
